"""Aggregate one benchmark for the validation workbook.

Usage: python scripts/benchmarks/summarize-climate-validation.py --run-id 159 --output DIR
...
The most recent earlier report accompanies the output as narrative context.
"""
...
def benchmark_file(run_id, name, root=ROOT):
    directory = root / 'runs/diagnostics/climate' / str(run_id)
    candidates = (directory / name, directory / 'maps' / name,
                  root / 'runs/diagnostics' / Path(name).stem / f'{run_id}.csv')
    return next((p for p in candidates if p.exists()), candidates[0])

def previous_report(run_id, directory=ROOT/'runs/reports'):
    candidates = [(int(p.stem), p) for p in directory.glob('*.md')
                  if p.stem.isdigit() and int(p.stem) < run_id]
    if not candidates:
        return None
    previous_id, path = max(candidates)
    return dict(run_id=previous_id, path=str(path), text=path.read_text(encoding='utf-8'))
...
def remap(a, ny, nx):
    sy, sx = a.shape[-2:]
    sb, tb = np.sin(np.deg2rad(bounds(sy))), np.sin(np.deg2rad(bounds(ny)))
...
def reference(name, ny, nx):
    ...
    if key not in REF_CACHE:
        ...
        assert h == w // 2
        REF_CACHE[key] = remap(a, ny, nx)
    return REF_CACHE[key]

def grid(ny, nx):
    assert ny == nx // 2
    b = bounds(ny)
    lat = 90 - (np.arange(ny) + .5) * 180 / ny
...
def regions(ny, nx, land, mask_key='output'):
    key = ny, nx, mask_key
    if key in REGION_CACHE:
        return REGION_CACHE[key]
    lat, lon, area = grid(ny, nx)
    ocean = 1-land
    b = bounds(ny)
    ...
    for item in AR6:
        fraction=np.clip(remap((source==int(item['id'])).astype(float),ny,nx),0,1)
...
def main():
    ...
    land=cell['land'][0]; reg=regions(ny,nx,land); masks,area,lat,lon=reg
    ...
    wy,wx,weather=load_field_table(run_file('weather_statistics.csv'))
    wland=np.clip(remap(land,wy,wx),0,1);wreg=regions(wy,wx,wland);wlat=wreg[2]
    windpairs=[]
    for layer,reflevel,label in [(0,'10m','Surface / 10 m proxy'),(1,'500','Upper / 500 hPa proxy')]:
        ...
        ru,rv=(quarter_means(reference(f'era5_{c}{reflevel}_monthly.uwclim',wy,wx)) for c in ['u','v'])
            ...
            if layer==0:
                for region,lo,hi,sign,beltname in [('N_SUBTROPICS',10,35,-1,'Trade-wind'),('S_SUBTROPICS',-35,-10,-1,'Trade-wind'),('N_MIDLAT',35,60,1,'Westerly'),('S_MIDLAT',-60,-35,1,'Westerly')]:
                    ...
                    def belt(a):
                        ...
                        b=bounds(wy);widths=np.maximum(0,np.minimum(b[:-1],hi)-np.maximum(b[1:],lo))
        ...
        scalar_ref=np.average(reference('worldclim_wind_monthly.uwclim',wy,wx),axis=0,weights=DAYS) if layer==0 else None
    ...
    py,px,process=load_field_table(run_file('climate_process_fields.csv'));process=process[0]
    preg=regions(py,px,np.clip(remap(land,py,px),0,1))
    tcwv=quarter_means(reference('era5_tcwv_monthly.uwclim',py,px))
    ascent=quarter_means(reference('era5_w500_ascent_monthly.uwclim',py,px))
    ...
    oy,ox,ocean=load_field_table(run_file('ocean_fields.csv'));ocean=ocean[0]
    wet=identified_ocean_mask(ocean);oreg=regions(oy,ox,1-wet.astype(float),'identified-ocean')
    ...
    registry=json.loads((ROOT/'runs/registry/climate.json').read_text())
    ...
    logpath=ROOT/'runs/logs/climate'/f'{args.run_id}.log'
    if not logpath.exists():logpath=args.output/'benchmark.log'
    detailed_coupling=[]
    if logpath.exists():
    ...
    payload=clean(dict(run_id=args.run_id,resolution=[nx,ny],wind_grid=[wx,wy],moisture_grid=[px,py],metadata=metadata,
        previous_report=previous_report(args.run_id),
    ...
    for name,items in [('validation_metrics',RECORDS),('validation_vectors',VECTORS)]:
        path=run_file(name+'.csv');path.parent.mkdir(parents=True,exist_ok=True)
