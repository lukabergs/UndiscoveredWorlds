...
def rank(metrics,baseline,ids):
    ...
    return dict(version=VERSION,baseline=baseline,best_in_batch=best,recommended_baseline=selected,
                ...
                formula='100 × weighted mean of dimensionless field losses; four seasons equally weighted. Scalar: .50 RMSE/reference P95–P05 spread + .20 error P90/same spread + .20 spatial-bin mismatch + .10 histogram total variation. Vector: .45 vector RMSE/reference RMS speed + .20 error P90/reference speed P90 + .20 direction error/180 + .15 speed-histogram total variation. Fixed common masks; calm candidate direction penalized 90 degrees where reference is active. Auxiliary fields, density mixed-layer depth and unavailable skin-temperature targets have zero weight. Ocean-current skill is an approximate comparison of effective-layer and near-surface velocities. No Köppen component. Ties retain the baseline.')
