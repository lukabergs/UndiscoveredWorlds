...
def remove_area_weighted_mean(values: np.ndarray) -> np.ndarray:
    edges = np.sin(np.deg2rad(latitude_edges(values.shape[0])))
...
def prepare_era5(source: Path, output: Path, width: int, height: int) -> list[dict[str, object]]:
    ...
    for source_variable, output_variable in ERA5_VARIABLES.items():
        ...
        latitudes = np.asarray(data.coords["lat"], dtype=np.float64)
        longitudes = np.asarray(data.coords["lon"], dtype=np.float64)
        ...
        for month in range(1, 13):
            ...
            values = resize(values, width, height, latitudes, longitudes)
...
def polygon_mask(
    ...
    def pixels(ring: list[list[float]]) -> list[tuple[float, float]]:
        return [
            (
                (longitude + 180.0) * width / 360.0 - 0.5,
                (90.0 - latitude) * height / 180.0 - 0.5,
            )
...
def main() -> int:
    ...
    parser.add_argument(
        "--era5",
        type=Path,
        default=Path("refs/source/era5-planette-2001-2020"),
    )
    parser.add_argument(
        "--regions",
        type=Path,
        default=Path("refs/source/ipcc-ar6-wgi-regions-v4/IPCC-WGI-reference-regions-v4.geojson"),
    )
    parser.add_argument("--output", type=Path, default=Path("refs/processed/climate"))
    parser.add_argument("--width", type=int, default=2048)
    parser.add_argument("--height", type=int, help="Must equal width/2; derived when omitted.")
    args = parser.parse_args()
    args.width, args.height = grid_dimensions(args.width, args.height)

    args.output.mkdir(parents=True, exist_ok=True)
    receipt = {
        "target_width": args.width,
        "target_height": args.height,
        "alignment": "global_equirectangular_cell_centres_north_to_south",
        "latitude_formula": "90 - (y + 0.5) * 180 / height",
        "processing": (
            "spherical-area conservative remapping from native ERA5 coordinates; "
            "raw endpoint samples are converted only while reading the source"
        ),
