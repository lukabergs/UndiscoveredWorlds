...
inline constexpr int height = 299;
...
inline constexpr float tempDecrease = 4.54f;
inline constexpr int northPolarAdjust = -4;
inline constexpr int southPolarAdjust = -4;
...
inline constexpr float gridScale(int width, int height)
{
    ...
    const float heightscale = static_cast<float>(height + 1) /
        static_cast<float>(worlddefaults::height + 1);
...
static_assert(scaleDistance(30, 2047, 1023) == 102);
}
...
inline constexpr double meridionalDiffusivityWm2K = 0.60;
inline constexpr double zonalLandOceanExchangeWm2K = 12.0;
inline constexpr double tropicalCloudAlbedoAmplitude = 0.06;
inline constexpr double tropicalCloudAlbedoWidthDegrees = 8.0;
...
// Reuse the existing warm-season ice diagnosis in coupled surface exchange.
inline constexpr bool couplePermanentLandIce = true;
...
inline constexpr float mixedLayerDepthMetres = 30.0f;
inline constexpr bool variableHeatStorage = true;
...
// The 300 m active layer has an interface, not a solid seafloor.
inline constexpr float linearBottomDragMps = 0.0f;
inline constexpr float heatDiffusivityM2S = 30000.0f;
...
inline constexpr float landMomentumDragCoefficient = 0.0180f;
inline constexpr float highReliefMomentumDragCoefficient = 0.0360f;
inline constexpr float frozenMomentumDragCoefficient = 0.012f;
inline constexpr float frozenSurfaceDragFraction = 1.0f;
inline constexpr float persistentSnowDragFraction = 1.0f;
// Optional prescribed vegetation contrast. Zero retains the previous closure.
    inline constexpr float vegetationDragContrastFraction = 12.0f;
// Zero disables drainage; nonzero is a reduced cold-layer/skin deficit ratio.
inline constexpr float katabaticDeficitFraction = 4.0f;
inline constexpr float katabaticRadiativeCoolingFraction = 1.333333333f;
inline constexpr float katabaticColdLayerRenewalSeconds = 21600.0f;
inline constexpr bool katabaticSnowPatchTemperature = true;
inline constexpr float katabaticMaximumDeficitK = 15.0f;
inline constexpr float katabaticMaximumSlope = 0.05f;
// Experimental vertical distribution of the diagnosed velocity increment.
// False reproduces the run-250 full-depth transport treatment.
inline constexpr bool separateKatabaticTransport = true;
inline constexpr float katabaticVelocityDecayHeightMetres = 75.0f;
// Cold-layer diagnostic experiments; false retains run 304's closure.
inline constexpr bool katabaticShallowTenMetreDiagnostic = true;
inline constexpr float katabaticShallowDepthMetres = 50.0f;
// Converts the inherited effective bulk stress calibration for the shallow
// increment only. It leaves the background circulation/land drag unchanged.
inline constexpr float katabaticTenMetreStressFraction = 0.166666667f;
inline constexpr bool katabaticReflectedPolarGradient = true;
static_assert(!katabaticShallowTenMetreDiagnostic || separateKatabaticTransport,
    "The shallow 10 m diagnostic requires separate column transport");
static_assert(!katabaticShallowTenMetreDiagnostic || (katabaticShallowDepthMetres > 20.0f &&
    katabaticShallowDepthMetres <= surfaceBoundaryLayerMomentumDepthMetres),
    "The diagnostic jet must contain 10 m and fit within the transport layer");
...
{
// Reversible trade-wind experiment; archived baseline leaves zonal pressure unadjusted.
inline constexpr bool adjustZonalSurfacePressure = true;
inline constexpr bool adjustZonalUpperHeight = true;
...
// Continental seasonal thermal anomalies may occupy a deeper column.
inline constexpr float surfaceThermalLandTopPressurePa = 40000.0f;
// Vertical mean fraction of the near-surface thermal anomaly (G9 used 0.5).
inline constexpr float surfaceThermalProfileFraction = 0.5f;
// Sensitivity of the continental thermal column; ocean profile remains separate.
inline constexpr float surfaceThermalLandProfileFraction = 0.5f;
// Couple the prognostic ground-temperature departure to the land thermal mode.
// This is a reduced-column calibration, not a replacement of air temperature.
inline constexpr float surfaceThermalLandSkinFraction = 0.0f;
inline constexpr float thermalPressureElevationCorrectionFraction = 1.0f;
inline constexpr float referenceTemperatureK = 288.0f;
inline constexpr float troposphereHeightMetres = 20000.0f; // Held-Hou overturning depth
inline constexpr float overturningMassRedistributionEfficiency = 0.060f;
// Reduced-circulation sensitivity; zero preserves the original subpolar low.
inline constexpr float subpolarLowDeepeningFraction = 0.5f;
inline constexpr bool subpolarDeepeningPreservePolarGradient = true;
inline constexpr float continentalSubpolarLowSuppression = 1.0f;
...
// Mechanical blocking depends on terrain and wind, not a tropical latitude gate.
inline constexpr float mechanicalTopographicMinimumLatitudeDegrees = 0.0f;
inline constexpr float mechanicalTopographicFullStrengthLatitudeDegrees = 0.0f;
...
inline constexpr float maximumThermalEquatorShiftHadleyFraction = 0.20f;
inline constexpr float winterHadleyEdgeAnchorFraction = 0.0f;
// S-series: move the tropical trough inside the existing subtropical highs.
inline constexpr float confinedThermalEquatorShiftHadleyFraction = 0.50f;
inline constexpr float regionalThermalTroughFraction = 1.0f;
inline constexpr float polarThermalContrastFraction = 1.0f;
// Calibrated negative eddy-lobe response; zero retains equal hemispheres.
inline constexpr float subpolarHemisphericContrastExponent = 2.0f;
...
// Picard updates reconcile the pressure inversion's linear drag with the
// diagnosed quadratic stress. The speed floor only regularizes calm cells.
inline constexpr int surfacePressureDragIterations = 8;
// Experimental bulk momentum transport; zero retains the algebraic closure.
inline constexpr int surfaceMomentumAdvectionIterations = 16;
// Auxiliary 850 hPa diagnosis alongside the retained surface closure.
inline constexpr bool enableAuxiliary850Momentum = true;
// The inversion mode need not inherit the surface pressure adjustment depth.
inline constexpr float auxiliary850ModeDepthMetres = 10000.0f;
inline constexpr float auxiliary850OverturningProjection = 0.4f;
inline constexpr float auxiliary850ZonalPressureFraction = 1.0f;
inline constexpr float auxiliary850HeatingResponseFraction = 1.0f;
inline constexpr float auxiliary850ThermalProfileFraction = 0.0f;
inline constexpr float surface850ExchangeRatePerSecond = 5.0e-6f;
inline constexpr float surfacePressureDragRelaxation = 0.5f;
inline constexpr float surfacePressureDragMinimumWindMps = 0.25f;
inline constexpr bool surfaceDragParallelMobility = true;
...
inline constexpr float upperModeDepthMetres = 10000.0f;
inline constexpr float stationaryWaveNondimensionalDamping = 0.05f;
...
inline constexpr bool upperOrographicBlocking = true;
// Calibration sensitivity after the bulk N*h/|U| launch-height limit.
inline constexpr float upperOrographicLaunchFraction = 0.5f;
...
// Advance stores with the coupled forcing instead of equilibrating them up to
// eight years before every forcing update. Minimum cycles counts carried years.
inline constexpr int yearsPerCouplingUpdate = 1;
...
// Experimental lower-reservoir proxy for ocean surface humidity; preserves the
// original column proxy unless an explicitly recorded experiment enables it.
inline constexpr bool oceanEvaporationUseBoundaryHumidity = true;
inline constexpr float minimumSurfaceWind = 1.0f;
inline constexpr float baselineOceanUnresolvedWindRmsMps = 2.0f;
...
// Fraction of the free-reservoir mode drawn from the diagnosed 850-hPa wind.
// Zero preserves the surface/500-hPa carrier; unavailable levels fall back.
inline constexpr float freeTroposphereLowLevelWindFraction = 0.0f;
...
inline constexpr float condensationConversionTimeDays = 0.25f;
