...
void clampworldpropertycontrols(WorldPropertyControls& controls)
{
    ...
    controls.mapWidth = std::clamp(controls.mapWidth, 2, ARRAYWIDTH) & ~1;
    controls.mapHeight = controls.mapWidth / 2;
...
{
    if (size == 0)
        return (tuning::worlddefaults::width + 1) / 4;

    if (size == 1)
        return (tuning::worlddefaults::width + 1) / 2;
...
{
    if (size == 0)
        return (tuning::worlddefaults::height + 1) / 4;

    if (size == 1)
        return (tuning::worlddefaults::height + 1) / 2;
