...
constexpr float particlespeeddisplaymaximum = static_cast<float>(climatepalette::field_surface_wind_high);
...
constexpr int maxflowvisualizationcolumns = 2048;
constexpr int maxflowvisualizationcells = 2048 * 1024;
constexpr int referencevisualizationversion = 4; // independent wind grid, cell-centred LIC latitude
...
struct RenderTimer
{
    ...
    ~RenderTimer()
    {
        std::cout << "Climate render kind=" << kind << " columns=" << columns << " rows=" << rows
...
seasonfields windrenderfields(const seasonfields& source, int columns, int rows, int targetcolumns)
{
    seasonfields result;
    const auto remap = [&](const std::vector<float>& input) {
        return climategrid::remapSmoothField(columns, rows, input, targetcolumns, targetcolumns / 2); };
    result.surfaceu = remap(source.surfaceu);
    result.surfacev = remap(source.surfacev);
    result.upperu = remap(source.upperu);
    result.upperv = remap(source.upperv);
    result.surfacespeed.resize(result.surfaceu.size());
    result.upperspeed.resize(result.upperu.size());
    for (std::size_t i = 0; i < result.surfaceu.size(); ++i)
    {
        result.surfacespeed[i] = std::hypot(result.surfaceu[i], result.surfacev[i]);
        result.upperspeed[i] = std::hypot(result.upperu[i], result.upperv[i]);
    }
    return result;
}
...
float latitudeforrow(int y, int rows)
{
    ...
    return static_cast<float>(
        climategrid::latitudeCentreRadians(y, rows) * radiansToDegrees);
}
...
{
    const bool isSigned = palette != scalarpalette::speed && palette != scalarpalette::vectorerror;
    const auto rgb = climatepalette::colour(value, isSigned ? -displaymaximum : 0.0, displaymaximum, isSigned);
...
sf::Color windbackgroundcolour(planet& world, int x, int y, float speed,
    int columns, int rows, float displaymaximum)
{
    x = std::clamp(static_cast<int>((x + 0.5) * (world.width() + 1) / columns), 0, world.width());
    y = std::clamp(static_cast<int>((y + 0.5) * (world.height() + 1) / rows), 0, world.height());
...
bool renderlicimage(
    ...
    int columns,
    int rows,
    float displaymaximum = particlespeeddisplaymaximum)
{
    const int imagecolumns = columns * licrenderscale;
    const int imagerows = rows * licrenderscale;
    const RenderTimer timer{"lic", path, imagecolumns, imagerows};
    ...
    for (int imagey = 0; imagey < imagerows; imagey++)
    {
        ...
        for (int imagex = 0; imagex < imagecolumns; imagex++)
        {
            ...
            sf::Color colour = windbackgroundcolour(world, cellx, celly, speed, columns, rows, displaymaximum);
    ...
    float displaymaximum = particlespeeddisplaymaximum)
{
    ...
    const RenderTimer timer{"particles", path, imagecolumns, imagerows};
    sf::Image image;
    image.create(
        static_cast<unsigned int>(imagecolumns),
        static_cast<unsigned int>(imagerows),
    ...
    for (int y = 0; y < rows; y++)
    {
        for (int x = 0; x < columns; x++)
        {
            ...
            const sf::Color background = windbackgroundcolour(world, x, y, speed, columns, rows, displaymaximum);
    ...
    for (int particle = 0; particle < particlecount; particle++)
    {
        ...
        float y = std::clamp(
            (90.0f - latitude) / 180.0f * static_cast<float>(rows) - 0.5f,
            0.0f,
            static_cast<float>(rows - 1));
...
seasonfields buildseasonfields(planet& world, int season)
{
    ...
    if (processworld == &world && processstatistics[season].durationSeconds > 0.0)
    {
        ...
        const auto remap = [&](const std::vector<float>& values) {
            return climategrid::remapSmoothField(
                processes.columns, processes.rows, values, columns, rows); };
    ...
    const climatebenchmarkmapselection& selection)
{
    ...
    const int columns = world.width() + 1;
    const int rows = world.height() + 1;
    const int windcolumns = selection.windmapcolumns();
    const int windrows = windcolumns / 2;
    std::ofstream windmetadata(outputdirectory / "wind_render_metadata.json");
    windmetadata << "{\"version\":1,\"columns\":" << windcolumns << ",\"rows\":" << windrows
        << ",\"source_columns\":" << columns << ",\"source_rows\":" << rows
        << ",\"renderer_version\":" << referencevisualizationversion
        << ",\"fields\":\"coarse seasonal winds; no terrain refinement\",\"products\":[\"lic\",\"particles\",\"speed\"]}\n";
    if (!windmetadata.good()) return false;
    std::cout << "Climate wind render grid columns=" << windcolumns << " rows=" << windrows
        << " source_columns=" << columns << " source_rows=" << rows << '\n';
    bool success = true;

    {
        std::ofstream layer(outputdirectory / "tropical_boundary_layer.csv");
        layer << "season,latitude,longitude,inversion_temperature_c,inversion_pressure_anomaly_hpa,inversion_u_mps,inversion_v_south_mps,thermal_pressure_hpa,surface_pressure_unblended_hpa,solver_residual\n";
        for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
        {
            if (!world.climatesimulation().mixedLayerTropics) break;
            const auto& state = world.climatesimulation().boundaryLayer[season];
            if (!state.inversionSolver.converged || state.surfacePressureHpa.empty()) { success = false; continue; }
            for (int y = 0; y < state.rows; ++y)
                for (int x = 0; x < state.columns; ++x)
                {
                    const int cell = y * state.columns + x;
                    layer << season << ',' << 90.0 - 180.0 * (y + 0.5) / state.rows << ','
                        << -180.0 + 360.0 * (x + 0.5) / state.columns << ',' << state.inversionTemperatureC[cell] << ','
                        << state.inversionPressureHpa[cell] << ',' << state.inversionEastWindMps[cell] << ','
                        << state.inversionSouthWindMps[cell] << ',' << state.thermalPressureHpa[cell] << ','
                        << state.surfacePressureHpa[cell] << ',' << state.inversionSolver.relativeResidual << '\n';
                }
        }
        success = layer.good() && success;
    }
    ...
    if (processworld == &world)
    {
        ...
        // Separate diagnostic file preserves the existing process CSV contract.
        std::ofstream terrain(outputdirectory / "climate_terrain_fields.csv");
        terrain << std::setprecision(12);
        terrain << "season,latitude,longitude,duration_s,stratiform_mm_day,orographic_mm_day,convective_mm_day,lower_rh,upper_rh,uplift_m,descent_m,terrain_cooling_c,terrain_warming_c\n";
        for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
        {
            const auto& p = processstatistics[season];
            if (p.durationSeconds <= 0.0 || p.upliftMetreSeconds.size() !=
                static_cast<std::size_t>(p.columns * p.rows)) continue;
            for (int y = 0; y < p.rows; ++y)
                for (int x = 0; x < p.columns; ++x)
                {
                    const int cell = y * p.columns + x;
                    terrain << season << ',' << 90.0 - 180.0 * (y + 0.5) / p.rows << ','
                        << -180.0 + 360.0 * (x + 0.5) / p.columns << ',' << p.durationSeconds;
                    for (const auto& rain : p.surfacePrecipitationMm)
                        terrain << ',' << rain[cell] * 86400.0 / p.durationSeconds;
                    for (const auto& humidity : p.relativeHumiditySeconds)
                        terrain << ',' << humidity[cell] / p.durationSeconds;
                    terrain << ',' << p.upliftMetreSeconds[cell] / p.durationSeconds
                        << ',' << p.descentMetreSeconds[cell] / p.durationSeconds
                        << ',' << p.terrainCoolingCSeconds[cell] / p.durationSeconds
                        << ',' << p.terrainWarmingCSeconds[cell] / p.durationSeconds << '\n';
                }
        }
        success = terrain.good() && success;
        std::ofstream thermal(outputdirectory / "climate_surface_thermal_fields.csv");
        thermal << std::setprecision(12);
        thermal << "season,latitude,longitude,skin_temperature_c,land_snow_cover,sensible_wm2,surface_radiative_wm2,surface_latent_loss_wm2,surface_net_heating_wm2,permanent_land_ice_cover\n";
        for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
        {
            const auto& p = processstatistics[season];
            if (p.durationSeconds <= 0.0 || p.landSnowCoverFraction.size() !=
                static_cast<std::size_t>(p.columns * p.rows)) continue;
            for (int y = 0; y < p.rows; ++y)
                for (int x = 0; x < p.columns; ++x)
                {
                    const int cell = y * p.columns + x;
                    thermal << season << ',' << 90.0 - 180.0 * (y + 0.5) / p.rows << ','
                        << -180.0 + 360.0 * (x + 0.5) / p.columns << ',' << p.surfaceTemperatureC[cell] << ','
                        << p.landSnowCoverFraction[cell] << ',' << p.sensibleHeatingWm2[cell] << ','
                        << p.surfaceRadiativeHeatingWm2[cell] << ',' << p.surfaceLatentHeatLossWm2[cell] << ','
                        << p.surfaceNetHeatingWm2[cell] << ','
                        << (p.permanentLandIceFraction.empty() ? 0.0f : p.permanentLandIceFraction[cell]) << '\n';
                }
        }
        success = thermal.good() && success;
    }
    if (oceanworld == &world)
    {
        std::ofstream ocean(outputdirectory / "ocean_fields.csv");
        std::ofstream storage(outputdirectory / "ocean_storage_fields.csv");
        storage << std::setprecision(12)
            << "season,latitude,longitude,storage_depth_m,mean_storage_depth_m,reservoir_temperature_c,entrainment_heat_wm2,column_storage_heat_wm2\n";
        ocean << std::setprecision(12);
        ocean << "season,latitude,longitude,east_current_mps,south_current_mps,sst_c,ekman_upwelling_mps,solver_converged,relative_heat_budget_residual,ice_thickness_m,surface_skin_temperature_c,mean_sst_c,depth_m,deep_temperature_c,horizontal_heat_wm2,surface_heat_wm2,vertical_heat_wm2,storage_heat_wm2,barotropic_east_mps,barotropic_south_mps,ekman_east_mps,ekman_south_mps,barotropic_upwelling_mps,ekman_only_upwelling_mps\n";
        for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
        {
            ...
            for (int y = 0; y < oceanrows; ++y)
                for (int x = 0; x < oceancolumns; ++x)
                {
                    ...
                    ocean << season << ',' << 90.0 - 180.0 * (y + 0.5) / oceanrows << ',' << -180.0 + 360.0 * (x + 0.5) / oceancolumns << ','
                        ...
                        << (state.meanSstC.size() > cell ? state.meanSstC[cell] : state.sstC[cell]);
                    const auto budget = state.heatBudget.size() > cell ? state.heatBudget[cell] : climateocean::OceanHeatBudget{};
                    if (state.storageDepthMetres.size() > cell && state.reservoirTemperatureC.size() > cell)
                        storage << season << ',' << 90.0 - 180.0 * (y + 0.5) / oceanrows << ','
                            << -180.0 + 360.0 * (x + 0.5) / oceancolumns << ',' << state.storageDepthMetres[cell] << ','
                            << state.meanStorageDepthMetres[cell] << ',' << state.reservoirTemperatureC[cell] << ','
                            << budget.entrainmentWm2 << ',' << budget.columnStorageWm2 << '\n';
                    ocean << ',' << budget.depthMetres << ',' << budget.deepTemperatureC << ','
                        << budget.horizontalWm2 << ',' << budget.surfaceWm2 << ','
                        << budget.verticalWm2 << ',' << budget.storageWm2;
                    const auto components = state.transportComponents.size() > cell ? state.transportComponents[cell] :
                        climateocean::OceanState::TransportComponents{};
                    ocean << ',' << components.barotropicEastMps << ',' << components.barotropicSouthMps << ','
                        << components.ekmanEastMps << ',' << components.ekmanSouthMps << ','
                        << components.barotropicUpwellingMps << ',' << components.ekmanUpwellingMps << '\n';
                }
        }
        success = ocean.good() && storage.good() && success;
    }
    ...
    for (int season = 0; season < CLIMATESEASONCOUNT; season++)
    {
        ...
        const bool upperspeed = selection.includes(
            climatebenchmarkmapkind::upperwindspeed, season);
        ...
        const seasonfields wind = (surfaceparticles || upperparticles || surfacelic || upperlic || surfacespeed || upperspeed)
            ? windrenderfields(fields, columns, rows, windcolumns) : seasonfields{};
        ...
        if (surfaceparticles && flowenabled)
        {
            success = renderparticleimage(
                ...
                wind.surfaceu,
                wind.surfacev,
                0x1185a11u + static_cast<std::uint32_t>(season) * 17u, windcolumns, windrows) && success;
        }
        ...
        if (upperparticles && flowenabled)
        {
            success = renderparticleimage(
                ...
                wind.upperu,
                wind.upperv,
                0x1185a91u + static_cast<std::uint32_t>(season) * 17u, windcolumns, windrows, static_cast<float>(climatepalette::field_500_wind_high)) && success;
        }
        ...
        if (surfacelic && flowenabled)
        {
            success = renderlicimage(
                ...
                wind.surfaceu,
                wind.surfacev,
                0x1185b11u + static_cast<std::uint32_t>(season) * 17u, windcolumns, windrows) && success;
        }
        ...
        if (upperlic && flowenabled)
        {
            success = renderlicimage(
                ...
                wind.upperu,
                wind.upperv,
                0x1185b91u + static_cast<std::uint32_t>(season) * 17u, windcolumns, windrows, static_cast<float>(climatepalette::field_500_wind_high)) && success;
        }
        ...
        {
            success = renderscalarimage(
                outputdirectory / diagnosticmapfilename(climatebenchmarkmapkind::surfacedivergence, season),
                fields.surfacedivergence, columns, rows, surfacedivergencedisplaymaximum, scalarpalette::divergence) && success;
        ...
        if (upperspeed)
        {
            ...
            windcolumns,
            windrows,
            static_cast<float>(climatepalette::field_500_wind_high),
            scalarpalette::speed) && success;
            renderstatus(climatebenchmarkmapkind::upperwindspeed, true, true);
        }
        ...
        for (const auto& request : selection.requests())
        {
            ...
            float limit = static_cast<float>(climatepalette::field_moisture_flux_high);
            ...
            const auto remap = [&](const std::vector<float>& input, int sourcecolumns, int sourcerows) {
                return climategrid::remapSmoothField(
                    sourcecolumns, sourcerows, input, columns, rows); };
            ...
            else if (oceanworld == &world)
            {
                ...
                {
                    const auto& climate = world.climatesimulation();
                    const auto& sst = climate.oceanSst[season];
                    if (climate.hasOcean && sst.size() == static_cast<std::size_t>(columns) &&
                        std::all_of(sst.begin(), sst.end(), [rows](const auto& column) {
                            return column.size() == static_cast<std::size_t>(rows); }))
                    {
                        // Show the same ocean-only reconstruction used by coupling.
                        values.resize(static_cast<std::size_t>(columns) * rows);
                        for (int y = 0; y < rows; ++y)
                            for (int x = 0; x < columns; ++x)
                                values[fieldindex(x, y, columns)] = sst[x][y];
                    }
                    else
                        values = remap(data.meanSstC.empty() ? data.sstC : data.meanSstC, oceancolumns, oceanrows);
                    limit = static_cast<float>(climatepalette::field_sst_high);
                    palette = scalarpalette::divergence;
                }
                else
                { u = remap(data.eastCurrentMps, oceancolumns, oceanrows); v = remap(data.southCurrentMps, oceancolumns, oceanrows); limit = static_cast<float>(climatepalette::field_current_high); }
            }
    ...
    const climatebenchmarkmapselection& selection)
{
    ...
    const std::string referenceprefix =
        ...
        std::to_string(selection.windmapcolumns()) + "x" + std::to_string(selection.windmapcolumns() / 2) +
        "_source" + std::to_string(columns) + "x" + std::to_string(rows);
    ...
    for (int season = 0; season < CLIMATESEASONCOUNT; season++)
    {
        ...
        for (int layer = 0; layer < 2; layer++)
        {
            ...
            const int windcolumns = selection.windmapcolumns(), windrows = windcolumns / 2;
            const bool needrender = (referenceparticlesrequested && !std::filesystem::exists(referenceparticlepath)) ||
                (referencelicrequested && !std::filesystem::exists(referencelicpath));
            const auto renderu = needrender ? climategrid::remapSmoothField(columns, rows, referenceu, windcolumns, windrows) : std::vector<float>{};
            const auto renderv = needrender ? climategrid::remapSmoothField(columns, rows, referencev, windcolumns, windrows) : std::vector<float>{};
                ...
                !std::filesystem::exists(referenceparticlepath))
            {
                success = renderparticleimage(
                    ...
                    renderu,
                    renderv,
                    particlebase + static_cast<std::uint32_t>(season) * 17u, windcolumns, windrows) && success;
            ...
            if (referencelicrequested && !std::filesystem::exists(referencelicpath))
            {
                success = renderlicimage(
                    ...
                    renderu,
                    renderv,
                    licbase + static_cast<std::uint32_t>(season) * 17u, windcolumns, windrows) && success;
                renderedreference = true;
            }
...
bool circulationflowvisualizationenabled(const planet& world)
{
    ...
    return columns <= maxflowvisualizationcolumns &&
        static_cast<long long>(columns) * rows <=
            maxflowvisualizationcells;
}
