...
{
    const auto rgb = climatepalette::colour(temperaturec, climatepalette::field_air_temperature_low, climatepalette::field_air_temperature_high, true);
    return sf::Color(rgb[0], rgb[1], rgb[2]);
}

sf::Color benchmarkprecipitationcolour(float precipitationmmyear)
{
    const auto rgb = climatepalette::colour(precipitationmmyear, climatepalette::field_annual_rain_low, climatepalette::field_annual_rain_high, false);
    return sf::Color(rgb[0], rgb[1], rgb[2]);
}
...
bool aligncategoricalreference(sf::Image& reference, const planet& world)
{
    ...
    for (unsigned int y = 0; y < rows; ++y)
        for (unsigned int x = 0; x < columns; ++x)
        {
            ...
            const unsigned int yy = (std::min)(size.y - 1,
                static_cast<unsigned int>((y + 0.5) * size.y / rows));
    ...
    int targetrows)
{
    ...
    const double sourcey = (static_cast<double>(y) + 0.5) *
        static_cast<double>(reference.height) / static_cast<double>(targetrows) - 0.5;
...
float latitudeforrow(int row, int height)
{
    ...
    return 90.0f - 180.0f * (static_cast<float>(row) + 0.5f) /
    ...
    const benchmarkphysicsmetrics& physics)
{
...

    {
        ...
        tempfile << content;
        tempfile.flush();
        if (!tempfile.good()) return false;
    }

    // Indexers/readers can briefly deny replacement on Windows. Keep the
    // completed registry entry recoverable if the bounded retry still fails.
    for (int attempt = 0; attempt < 10; ++attempt)
    {
        if (MoveFileExW(temppath.wstring().c_str(), logpath.wstring().c_str(), MOVEFILE_REPLACE_EXISTING | MOVEFILE_WRITE_THROUGH))
            return true;
        const DWORD error = GetLastError();
        if (attempt == 9 || (error != ERROR_SHARING_VIOLATION && error != ERROR_LOCK_VIOLATION && error != ERROR_ACCESS_DENIED))
        {
            cerr << "Registry replacement failed (Windows error " << error << "); completed entry retained at " << temppath.string() << '\n';
            return false;
        }
        Sleep(100);
    }
    ...
    const climatebenchmarkmapselection& selection)
{
    ...
    const auto saveimage = [&](const sf::Image& image, climatebenchmarkmapkind kind)
    {
        ...
        const filesystem::path path = outputdir /
            climatebenchmarkmaprelativepath(request, runid);
        filesystem::create_directories(path.parent_path(), filesystemerror);
        if (filesystemerror || !image.saveToFile(path.string()))
        {
        ...
        selection.includes(climatebenchmarkmapkind::precipitationtiff))
    {
        ...
        if (selection.includes(climatebenchmarkmapkind::precipitationtiff))
        {
            ...
            const filesystem::path path = outputdir.parent_path() / "fields" /
                climatebenchmarkmaprelativepath(request, runid);
            filesystem::create_directories(path.parent_path(), filesystemerror);
            if (filesystemerror)
                return false;
    ...
    for (const climatebenchmarkmaprequest& request : selection.requests())
    {
        ...
        if (!filesystem::exists(source))
        {
            mapmanifest << climatebenchmarkmapid(request) << ','
                << destination.generic_string() << ",skipped\n";
            continue;
        }

        filesystemerror.clear();
        filesystem::create_directories(destination.parent_path(), filesystemerror);
        if (filesystemerror)
            return false;
        ...
        mapmanifest << climatebenchmarkmapid(request) << ','
            << destination.generic_string() << ",written\n";
    }
    ...
    guide
        << "BENCHMARK MAP GUIDE\n\n"
        << "Canonical maps: runs/maps/climate/<quantity>/<style>/<season>/<layer>/<ID>.png.\n"
        << "Season folders are 1=Jan, 2=Apr, 3=Jul, 4=Oct; annual products use annual.\n"
        << "Rain GeoTIFFs are in runs/fields/climate/rain/annual/s/<ID>.tif.\n"
        << "See runs/maps/climate/README.md and refs/processed/climate/maps/README.md for paths and palettes.\n"
        << "The legacy names below identify diagnostic staging files only.\n\n"
        << "All maps are global equirectangular grids with north at the top and periodic longitude.\n\n"
        << "Wind speed, LIC and particles use --wind-map-resolution (default 2048); scalar products keep world dimensions.\n"
        << "These native wind maps show coarse seasonal circulation; W2/Q3 terrain refinement is a separate diagnostic pass.\n\n"
        << "{ID}_koppen.png: categorical land climate class; colors follow the workbook's Koeppen-Geiger legend; water is black.\n"
        << "Shared colours and fixed scales: configs/climate-palettes.json (climate-colour-v2).\n"
        << "Positive magnitudes: blue/cyan/green/yellow/orange/red. Signed fields: white/magenta/blue/cyan/green/yellow/red; zero cyan. Black is masked/missing.\n"
        << "{ID}_temp.png: four-season mean land surface-air temperature, signed scale -54 to +40 C; water black.\n"
        << "{ID}_precip.png: annual precipitation, positive scale 0 to 6000+ mm/year.\n"
        ...
        << "Upper-wind variants use u_wind. LIC/particle backgrounds encode speed on 0-25 m/s surface and 0-40 m/s upper scales, modulated by texture.\n"
        << "{ID}_{season}_s_wind_err.png: |simulation minus ERA5|; positive scale 0-25 m/s; arrows show error direction.\n"
        << "{ID}_{season}_s_wind_speed.png: scalar wind speed with the same 0-to-25+ m/s palette.\n"
        << "{ID}_{season}_u_wind_speed.png: upper scalar wind speed, positive scale 0-40 m/s.\n"
        << "{ID}_{season}_s_div.png: divergence per day; signed scale +/-1 day^-1.\n"
        << "{ID}_{season}_moist_conv.png: moisture-flux convergence; shared signed scale +/-20 mm/day, zero cyan.\n"
        ...
        << "{ID}_{season}_{b_moist_flux|f_moist_flux|moist_flux}.png: lower, upper, or summed moisture transport; arrows show direction; shared positive scale 0-800 kg m^-1 s^-1.\n"
        << "{ID}_{season}_ascent.png: mean transport-stage ascent; shared signed scale +/-100 hPa/day, upward positive.\n"
        << "{ID}_{season}_heating.png: net atmospheric radiative+sensible+phase-change heating; shared signed scale +/-300 W/m2.\n"
        ...
        << "{ID}_{season}_ocean_current.png: current direction arrows; shared positive scale 0-1 m/s; land black.\n"
        << "{ID}_{season}_sst.png: liquid mixed-layer temperature (freezing floor -1.8 C); shared signed scale +/-35 C, zero cyan; land black. Ice-skin temperature and thickness are separate ocean_fields.csv columns.\n"
    ...
    for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
        for (int y = 0; y <= world.height(); ++y)
            for (int x = 0; x <= world.width(); ++x)
            {
                ...
                output << x << ',' << y << ','
                    << 90.0 - 180.0 * (y + 0.5) / (world.height() + 1) << ','
