...
int encodenativeheight(float value, uint16_t sealevelmeters)
{
    return TopographyCodec::internal_to_meters(value, sealevelmeters);
}

void applynativeheightmap(planet& world, std::vector<std::vector<bool>>& shelves, const float* outputheightmap, std::size_t cellcount, uint16_t sealevelmeters)
{
    if (outputheightmap == nullptr || cellcount == 0)
        return;

    const int width = world.width();
    const int height = world.height();
    const int simwidth = width + 1;
    const int sealevel = sealevelmeters;
    const int maxelevation = int(TopographyCodec::kMaxHeightMeters) + 1;
    ...
    parallelforrows(0, height, [&](int startrow, int endrow)
    {
        for (int y = startrow; y <= endrow; y++)
        {
            for (int x = 0; x <= width; x++)
            {
                ...
                world.setnom(x, y, encodenativeheight(outputheightmap[index], sealevelmeters));
                shelves[x][y] = false;
            }
        }
    });
...
void applyplatetectonicssimulation(planet& world, std::vector<std::vector<bool>>& shelves, const PlateTectonicsSimulationOptions& options)
{
    ...
    scenario.gravity_mps2 = std::isfinite(world.gravity()) && world.gravity() > 0.0f
        ? world.gravity() * platec::scenario::kDefaultGravityMps2
    ...
    while (platec_api_is_finished(simulation.pointer) == 0)
    {
        ...
        if (previewheightmap != nullptr)
        {
            applynativeheightmap(world, shelves, previewheightmap, cellcount,
                platec_api_get_sea_level_m(simulation.pointer));
