...
const uint8_t* platec_api_get_surface_crust_type_map(void* pointer) {
    lithosphere* litho = static_cast<lithosphere*>(pointer);
    return litho->getSurfaceCrustTypeMap();
}
...
const float* platec_api_get_tension_map(void* pointer) {
    lithosphere* litho = static_cast<lithosphere*>(pointer);
    return litho->getTensionMap();
}
...
uint32_t platec_api_get_tectonic_junction_count(void* pointer) {
    lithosphere* litho = static_cast<lithosphere*>(pointer);
    return litho->getTectonicJunctionCount();
}

const platec::contract::TectonicJunction* platec_api_get_tectonic_junctions(void* pointer) {
    lithosphere* litho = static_cast<lithosphere*>(pointer);
    return litho->getTectonicJunctions();
}

const uint8_t* platec_api_get_material_map(void* pointer) {
    lithosphere* litho = static_cast<lithosphere*>(pointer);
    return litho->getMaterialMap();
}
...
uint8_t platec_api_is_oceanic_plate(void* pointer, uint32_t plate_index) {
    lithosphere* litho = static_cast<lithosphere*>(pointer);
    return litho->getPlate(plate_index)->isOceanicPlate() ? 1U : 0U;
}
