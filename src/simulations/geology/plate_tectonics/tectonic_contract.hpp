...
inline constexpr uint32_t kPhase6SchemaVersion = 6;
...
inline constexpr uint32_t kNoTectonicJunctionId = 0;
...
enum class JunctionArmType : uint8_t {
    Unknown = 0,
    Ridge = 1,
    Trench = 2,
    Fault = 3,
...
enum class JunctionStability : uint8_t {
    Unknown = 0,
    Stable = 1,
    ConditionallyStable = 2,
    Unstable = 3,
    Indeterminate = 4,
};
...
struct TectonicJunction {
    uint32_t id = kNoTectonicJunctionId;
    uint32_t plate_ids[3] = {0, 0, 0};
    uint32_t boundary_segment_ids[3] = {kNoBoundaryId, kNoBoundaryId, kNoBoundaryId};
    JunctionArmType arm_types[3] = {JunctionArmType::Unknown, JunctionArmType::Unknown,
                                    JunctionArmType::Unknown};
    float arm_strike_x[3] = {0.0f, 0.0f, 0.0f};
    float arm_strike_y[3] = {0.0f, 0.0f, 0.0f};
    float arm_orientation_quality[3] = {0.0f, 0.0f, 0.0f};
    uint32_t candidate_cell_count = 0;
    float centroid_x = 0.0f;
    float centroid_y = 0.0f;
    float junction_velocity_x = 0.0f;
    float junction_velocity_y = 0.0f;
    // Historical name retained: this is the normalized RMS arm velocity residual.
    float velocity_closure_error = 0.0f;
    float velocity_quality_score = 0.0f;
    float velocity_constraint_error = 0.0f;
    float geometry_quality_score = 0.0f;
    JunctionStability stability = JunctionStability::Unknown;
    float stability_score = 0.0f;
    float quality_score = 0.0f;
};

struct Snapshot {
    uint32_t schema_version = kPhase6SchemaVersion;
    uint32_t width = 0;
    uint32_t height = 0;
    ...
    std::vector<TectonicJunction> tectonic_junctions;
