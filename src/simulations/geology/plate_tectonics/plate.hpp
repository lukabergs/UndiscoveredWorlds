...
class plate : public IPlate {
  public:
    ...
    plate(long seed, float* m, uint32_t w, uint32_t h, uint32_t _x, uint32_t _y, uint32_t plate_age,
          WorldDimension worldDimension, const uint8_t* local_material_map = nullptr,
          float erosion_strength = 1.0f,
          float crust_rotation_strength = 0.0f, float rotation_strength = 1.0f,
          float movement_energy = 1.0f);
    ...
    void addCrustByCollision(uint32_t x, uint32_t y, float z, uint32_t t,
                             ContinentId activeContinent, int material_index = -1);
    ...
    void setCrustTimestamp(uint32_t x, uint32_t y, uint32_t t);
    uint8_t getMaterial(uint32_t x, uint32_t y) const;
    float getSubmergedCrust(uint32_t x, uint32_t y) const;
    uint32_t getSubmergedSince(uint32_t x, uint32_t y) const;
    uint8_t getSubmergedMaterial(uint32_t x, uint32_t y) const;
    void submergeCrust(uint32_t x, uint32_t y, float z, uint32_t crust_age,
                       uint32_t hidden_since, int material_index = -1);
    float resurfaceSubmergedCrust(uint32_t x, uint32_t y, float fraction, float min_amount,
    ...
    void getMap(const float** c, const uint32_t** t, const uint8_t** m = nullptr) const;
    ...
    /// @param  t   Time of creation of new crust.
    void setCrust(uint32_t x, uint32_t y, float z, uint32_t t, int material_index = -1);
    ...
    void setKinematics(const Platec::FloatVector& linear_velocity, float angular_velocity) {
        _movement.setKinematics(linear_velocity, angular_velocity);
    }
    PlateType plateType() const noexcept;
    bool isOceanicPlate() const noexcept;
    bool isContinentalPlate() const noexcept;
    float continentalAreaRatio() const noexcept;
    float buoyancy() const noexcept {
        return _buoyancy;
    }
    float continentalityScore() const noexcept;
    void forcePlateType(PlateType type) noexcept;
    ...
    bool hasSubmergedCrust() const noexcept {
        return _submerged_mass > 1.0e-6f;
    }
    float getCx() const {
        return _mass.getCx();
    }
  ...
  private:
    ...
    MaterialMap material_map; ///< Bitmap of plate material indices.
    HeightMap submerged_map; ///< Hidden subducted crust that can later reemerge.
    AgeMap submerged_age_map;
    AgeMap submerged_since_map;
    MaterialMap submerged_material_map;
    IBounds* _bounds;
    Mass _mass;
    Movement _movement;
    PlateType _plate_type;
    float _buoyancy;
    float _submerged_mass;
    float _continental_area_ratio;
    float _continental_mass_ratio;
    float _mean_crust;
    float _erosion_strength;
    float _crust_rotation_strength;
    float _movement_energy;
