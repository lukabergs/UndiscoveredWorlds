...
float wrap_coordinate(float value, uint32_t period) {
    if (period == 0U) {
        return 0.0f;
    }
    ...
    float wrapped = std::fmod(value, period_f);
    if (wrapped < 0.0f) {
        wrapped += period_f;
    }
    if (wrapped >= period_f) {
...
void FloatPoint::shift(float dx, float dy, const WorldDimension& _worldDimension) {
    _x = wrap_coordinate(_x + (std::isfinite(dx) ? dx : 0.0f), _worldDimension.getWidth());
    _y = wrap_coordinate(_y + (std::isfinite(dy) ? dy : 0.0f), _worldDimension.getHeight());
