...
#include "detail/process_time_step.hpp"
#include "detail/upper_thermal_projection.hpp"
#include "detail/circulation_capture.hpp"
...
#include "detail/surface_evaporation_humidity.hpp"
#include "detail/frozen_surface_drag.hpp"
#include "detail/vegetation_drag.hpp"
#include "../../io/climate_surface_input.hpp"
#include "../../io/climate_experiment_input.hpp"
...
using hydrologyfloatgrid = climatecontext::RowMajorGrid<float>;
using hydrologybytegrid = climatecontext::RowMajorGrid<signed char>;
constexpr std::array<float, CLIMATESEASONCOUNT> seasonlatitudephase = { -1.0f, 0.0f, 1.0f, 0.0f };

const climateio::experiment::Inputs& referenceexperiment()
{
    static const climateio::experiment::Inputs inputs([] {
        const char* directory = std::getenv("UW_CLIMATE_EXPERIMENT_INPUT");
        if (directory && *directory)
            std::cout << "REFERENCE-FORCED DIAGNOSTIC: " << directory << '\n';
        return directory && *directory ? std::filesystem::path(directory) : std::filesystem::path{};
    }());
    return inputs;
}

float worldpermanentlandicefraction(const planet& world, int x, int y)
{
    if (!tuning::climate::energybalance::couplePermanentLandIce || world.sea(x,y)) return 0.0f;
    int warmest = world.seasonaltemp(0,x,y);
    for (int season = 1; season < CLIMATESEASONCOUNT; ++season)
        warmest = std::max(warmest, world.seasonaltemp(season,x,y));
    return climateenergy::permanentLandIceFraction(static_cast<float>(warmest));
}
...
float latitudeforrow(int y, int height)
{
    ...
    return static_cast<float>(
        climategrid::latitudeCentreRadians(y, height + 1) * radiansToDegrees);
}
...
double rowareaweight(int y, int height)
{
    ...
    const double halfstep = 0.5 * pi / static_cast<double>(rows);
    return climategrid::latitudeBandMeasure(y, rows) /
        (2.0 * std::sin(halfstep));
}
...
float samplehydrologygrid(
    ...
    const climategrid::ConservativeRemap& remap,
    int targetX,
    int targetY)
{
    return climategrid::sampleConservative(remap, targetX, targetY,
        [&](int x, int y) { return field[x][y]; });
}
...
{
    auto& climate = world.climatesimulation();
    ...
    const auto dimensions = climatehydrology::climateGridDimensions(
        outputcolumns,
        outputrows,
        climate.resolution.oceanColumns);
    ...
    const auto inputRemap = climategrid::makeConservativeRemap(outputcolumns, outputrows, columns, rows);
    const auto outputRemap = climategrid::makeSmoothRemap(columns, rows, outputcolumns, outputrows);
    auto reservoir = climate.oceanStorage;
    if (reservoir.columns != columns || reservoir.rows != rows)
        reservoir = {columns, rows};
    std::array<climateocean::OceanForcing, CLIMATESEASONCOUNT> forcingBySeason;
    std::array<climateocean::OceanConfig, CLIMATESEASONCOUNT> configs;
    std::array<std::vector<float>, CLIMATESEASONCOUNT> surfaceRadiation, surfaceLatentLoss, surfaceSensibleLoss;
    std::vector<float> outputland(outputcellcount, 0.0f);
    std::vector<std::uint8_t> outputlandmask(outputcellcount, 0);
    std::vector<float> hydrologyland(outputcellcount, 0.0f);
    for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
    {
        auto& config = configs[season];
        ...
        for (int y = 0; y < outputrows; y++)
        {
            for (int x = 0; x < outputcolumns; x++)
            {
                ...
                outputlandmask[cell] = land ? 1 : 0;
                hydrologyland[cell] = world.sea(x, y) ? 0.0f : 1.0f;
                outputdepth[cell] = land
                    ? 0.0f
                    : static_cast<float>(std::max(50, sealevel - world.nom(x, y)));
                const bool cached = climate.winds.populated[season];
                outputeastwind[cell] = cached ? climate.winds.surfacewindu[season][x][y] : static_cast<float>(world.seasonaluwind(season, x, y));
                outputsouthwind[cell] = cached ? climate.winds.surfacewindv[season][x][y] : static_cast<float>(world.seasonalvwind(season, x, y));
                ...
                outputfluxreference[cell] = climate.hasOcean && !climate.oceanSkin[season].empty()
                    ? climate.oceanSkin[season][x][y] : outputtemperature[cell];
            }
        }
        ...
        const auto remapinput = [&](const std::vector<float>& field)
        {
            return climategrid::remapField(
                inputRemap, field, outputcolumns);
        };
        const auto remapwetinput = [&](const std::vector<float>& field)
        {
            return climateocean::detail::remapOceanThermalField(
                inputRemap, outputlandmask, field, std::vector<float>(cellcount, 0.0f));
        };
        auto& forcing = forcingBySeason[season];
        ...
        // These intensive fields describe the water portion of a mixed cell.
        // Land depth placeholders and land winds/temperatures are not donors.
        forcing.bathymetryMetres = remapwetinput(outputdepth);
        // Solve the wind-driven gyre above a quiescent abyss. This is a
        // fixed active-layer approximation, not a prognostic thermocline.
        // The diagnostic depth now reports this dynamical column; world
        // bathymetry is retained. A 300 m cap preserves the thermal column.
        if (tuning::climate::oceancurrents::windDrivenActiveDepthMetres > 0.0f)
            for (float& depth : forcing.bathymetryMetres)
                depth = std::min(depth, tuning::climate::oceancurrents::windDrivenActiveDepthMetres);
        forcing.eastWindMps = remapwetinput(outputeastwind);
        forcing.southWindMps = remapwetinput(outputsouthwind);
        forcing.atmosphericTemperatureC = remapwetinput(outputtemperature);
        forcing.initialSstC = remapwetinput(outputsst);
        for (std::size_t cell = 0; cell < cellcount; ++cell)
            if (!forcing.landMask[cell]) forcing.initialSstC[cell] = std::max(config.freezingTemperatureC, forcing.initialSstC[cell]);
        forcing.initialIceThicknessMetres = remapwetinput(outputice);
        if (reservoir.sstC.empty())
        {
            reservoir.sstC = forcing.initialSstC;
            reservoir.iceMetres = forcing.initialIceThicknessMetres;
            reservoir.deepTemperatureC = forcing.initialSstC;
            for (auto& value : reservoir.deepTemperatureC)
                value = std::max(config.freezingTemperatureC, value - config.deepWaterTemperatureContrastK);
        }
        forcing.initialSstC = reservoir.sstC;
        forcing.initialIceThicknessMetres = reservoir.iceMetres;
        forcing.deepWaterTemperatureC = reservoir.deepTemperatureC;
        forcing.initialStorageDepthMetres = reservoir.storageDepthMetres;
        forcing.initialReservoirTemperatureC = reservoir.reservoirTemperatureC;
        forcing.surfaceHeatFluxReferenceTemperatureC = remapwetinput(outputfluxreference);
        if (climate.hasProcesses && climate.processes[season].durationSeconds > 0.0)
        {
            const auto& fields = climate.processes[season];
            // Match the binary sea-cell diagnosis used to compute these fluxes.
            // Remap flux and its linearization temperature with the SAME wet
            // support; mixing land fluxes changes the ocean boundary condition.
            const auto processlandfraction = climategrid::remapField(
                outputcolumns, outputrows, hydrologyland, fields.columns, fields.rows);
            std::vector<std::uint8_t> processland(processlandfraction.size());
            for (std::size_t cell = 0; cell < processland.size(); ++cell)
                processland[cell] = processlandfraction[cell] >= 0.5f ? 1 : 0;
            const auto processRemap = climategrid::makeConservativeRemap(
                fields.columns, fields.rows, columns, rows);
            std::vector<float> fallbackflux(cellcount);
            for (std::size_t cell = 0; cell < cellcount; ++cell)
                fallbackflux[cell] = tuning::climate::oceancurrents::surfaceHeatExchangeWm2K *
                    (forcing.atmosphericTemperatureC[cell] - forcing.surfaceHeatFluxReferenceTemperatureC[cell]);
            forcing.surfaceHeatFluxWm2 = climateocean::detail::remapOceanThermalField(
                processRemap, processland, fields.surfaceNetHeatingWm2, fallbackflux);
            forcing.surfaceHeatFluxReferenceTemperatureC = climateocean::detail::remapOceanThermalField(
                processRemap, processland, fields.surfaceTemperatureC, forcing.surfaceHeatFluxReferenceTemperatureC);
            const std::vector<float> missing(cellcount, 0.0f);
            surfaceRadiation[season] = climateocean::detail::remapOceanThermalField(
                processRemap, processland, fields.surfaceRadiativeHeatingWm2, missing);
            surfaceLatentLoss[season] = climateocean::detail::remapOceanThermalField(
                processRemap, processland, fields.surfaceLatentHeatLossWm2, missing);
            surfaceSensibleLoss[season] = climateocean::detail::remapOceanThermalField(
                processRemap, processland, fields.sensibleHeatingWm2, missing);
        }
        ...
        config.minimumCoriolisPerSecond = tuning::climate::oceancurrents::ekmanDampingPerSecond;
        config.linearBottomDragMps = tuning::climate::oceancurrents::linearBottomDragMps;
        config.mixedLayerDepthMetres =
            tuning::climate::oceancurrents::mixedLayerDepthMetres;
        config.variableHeatStorage = tuning::climate::oceancurrents::variableHeatStorage;
        config.storageStratificationPerSecond2 = tuning::climate::oceancurrents::storageStratificationPerSecond2;
        config.storageWindMixingEfficiency = tuning::climate::oceancurrents::storageWindMixingEfficiency;
        config.minimumStorageDepthMetres = tuning::climate::oceancurrents::minimumStorageDepthMetres;
        ...
        config.oneWay = true;
        const int hours = std::clamp(climate.oceanHeatStepHours, 1, 24);
        const int stepsPerDay = (24 + hours - 1) / hours;
        config.oceanTimeStepSeconds = 86400.0f / stepsPerDay;
        config.reuseSeasonalCirculation = climate.reuseOceanCirculation;
        config.krylovCirculation = climate.krylovOceanCirculation;
        config.gatherHeatFluxes = climate.gatherOceanHeatFluxes;
        config.heatWorkers = climate.oceanHeatWorkers;
        config.coastalScheme = climate.oceanCoastalScheme;
        config.heatStepsPerIteration = 0;
        for (int month = season * 3; month < season * 3 + 3; ++month)
            config.heatStepsPerIteration += climatehydrology::calendarMonth(month).days * stepsPerDay;
    }
    if (!climate.oceanCaptureDirectory.empty())
        for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
            std::cout << "Ocean capture path=" << climateocean::detail::OceanReplay{
                columns, rows, season, configs[season], forcingBySeason[season]}.write(climate.oceanCaptureDirectory).string() << '\n';
    auto periodic = climateocean::solvePeriodicOcean(columns, rows, forcingBySeason, configs);
    if (periodic.accepted)
    {
        reservoir.sstC = periodic.seasons.back().sstC;
        reservoir.iceMetres = periodic.seasons.back().iceThicknessMetres;
        if (configs[0].variableHeatStorage)
        {
            reservoir.storageDepthMetres = periodic.seasons.back().storageDepthMetres;
            reservoir.reservoirTemperatureC = periodic.seasons.back().reservoirTemperatureC;
        }
        reservoir.annualEnthalpyDriftK = periodic.annualEnthalpyDriftK;
        reservoir.periodic = periodic.converged;
        climate.oceanStorage = std::move(reservoir);
    }
    else climate.oceanStorage.periodic = false;
    std::cout << "Ocean spinup years=" << periodic.years << " annual_enthalpy_drift_k="
        << periodic.annualEnthalpyDriftK << " periodic=" << periodic.converged << '\n';
    std::cout << "Ocean profile columns=" << columns << " rows=" << rows
        << " circulation_seconds=" << periodic.circulationSeconds << " heat_seconds=" << periodic.heatSeconds
        << " zonal_seconds=" << periodic.zonalHeatSeconds
        << " flux_seconds=" << periodic.heatFluxSeconds << " update_seconds=" << periodic.heatUpdateSeconds
        << " heat_kernel=" << (climate.gatherOceanHeatFluxes ? "gather-v1" : "scatter-v1")
        << " heat_workers=" << periodic.seasons[0].heatWorkersUsed
        << " coastal_scheme=" << static_cast<int>(climate.oceanCoastalScheme)
        << " circulation_solves=" << periodic.circulationSolves << " heat_steps=" << periodic.heatSubsteps
        << " step_hours=" << climate.oceanHeatStepHours << " reuse=" << climate.reuseOceanCirculation
        << " solver=" << (climate.krylovOceanCirculation ? "bicgstab-sgs-v1" : "relaxation-v1") << '\n';
    for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
    {
        auto& ocean = periodic.seasons[season];
        const auto& config = configs[season];
        const auto& forcing = forcingBySeason[season];
        climate.oceanAccepted[season] = climateocean::usableOceanState(ocean, cellcount);
        const auto& limit = ocean.stabilityLimit;
        if (limit.cell >= 0)
            std::cout << "Ocean timestep season=" << season << " columns=" << columns << " rows=" << rows
                << " cell=" << limit.cell << " longitude=" << (-180.0 + 360.0 * (limit.cell % columns + 0.5) / columns)
                << " latitude=" << (90.0 - 180.0 * (limit.cell / columns + 0.5) / rows)
                << " rate_s=" << limit.totalRate << " stable_seconds=" << (0.7 / limit.totalRate)
                << " actual_seconds=" << ocean.actualHeatStepSeconds << " zonal_implicit=" << config.implicitZonalHeatDiffusion
                << " zonal_advection_s=" << limit.zonalAdvectionRate << " meridional_advection_s=" << limit.meridionalAdvectionRate
                << " zonal_diffusion_s=" << limit.zonalDiffusionRate << " meridional_diffusion_s=" << limit.meridionalDiffusionRate
                << " vertical_s=" << limit.verticalRate << " surface_s=" << limit.surfaceRate << '\n';
        const auto& distribution = ocean.stabilityDistribution;
        std::cout << "Ocean rate distribution season=" << season << " wet_cells=" << distribution.wetCells
            << " wet_area_m2=" << distribution.wetAreaM2
            << " p50_rate_s=" << distribution.rateQuantiles[0] << " p95_rate_s=" << distribution.rateQuantiles[1]
            << " p99_rate_s=" << distribution.rateQuantiles[2]
            << " without_advection_rate_s=" << distribution.maximumRateWithoutAdvection << '\n';
        constexpr std::array<int, 5> diagnosticHours{1, 3, 6, 12, 24};
        for (std::size_t bin = 0; bin < diagnosticHours.size(); ++bin)
            std::cout << "Ocean rate threshold season=" << season << " hours=" << diagnosticHours[bin]
                << " cells=" << distribution.restrictedCells[bin]
                << " area_fraction=" << (distribution.restrictedAreaM2[bin] / std::max(1.0, distribution.wetAreaM2)) << '\n';
        constexpr std::array<const char*, 4> faceNames{"east", "west", "south", "north"};
        for (std::size_t rank = 0; rank < distribution.highestRates.size(); ++rank)
        {
            const auto& hot = distribution.highestRates[rank];
            if (hot.cell < 0) continue;
            std::cout << "Ocean hotspot season=" << season << " rank=" << rank + 1 << " cell=" << hot.cell
                << " longitude=" << (-180.0 + 360.0 * (hot.cell % columns + 0.5) / columns)
                << " latitude=" << (90.0 - 180.0 * (hot.cell / columns + 0.5) / rows)
                << " rate_s=" << hot.totalRate << " depth_m=" << hot.depthMetres
                << " zonal_advection_s=" << hot.zonalAdvectionRate << " meridional_advection_s=" << hot.meridionalAdvectionRate
                << " vertical_s=" << hot.verticalRate;
            for (std::size_t face = 0; face < faceNames.size(); ++face)
                std::cout << ' ' << faceNames[face] << "_mps=" << hot.faceVelocityMps[face]
                    << ' ' << faceNames[face] << "_barotropic_mps=" << hot.barotropicFaceVelocityMps[face]
                    << ' ' << faceNames[face] << "_ekman_mps=" << hot.faceVelocityMps[face] - hot.barotropicFaceVelocityMps[face]
                    << ' ' << faceNames[face] << "_depth_m=" << hot.faceDepthMetres[face];
            std::cout << '\n';
        }
        if (!climate.oceanAccepted[season])
        {
        ...
        if (!climate.oceanAccepted[season])
        {
            ocean.meanSstC = ocean.sstC;
            ocean.meanSurfaceSkinTemperatureC = ocean.surfaceSkinTemperatureC;
            ocean.meanIceCover = ocean.iceThicknessMetres;
            for (auto& h : ocean.meanIceCover) h = std::clamp(h / 0.10f, 0.0f, 1.0f);
        }
        climatevalidation::captureoceanfields(world, season, columns, rows, ocean);
        // Log the forcing actually used, before the next hydrology update.
        // Sequential groups of four seasons identify outer ocean calls.
        if (climate.oceanAccepted[season] && !surfaceRadiation[season].empty())
            for (int band = 0; band < 4; ++band)
            {
                std::array<double, 11> sums{};
                double area = 0.0, maximumClosure = 0.0;
                int cells = 0;
                for (int y = 0; y < rows; ++y)
                    for (int x = 0; x < columns; ++x)
                    {
                        const int cell = y * columns + x;
                        const double latitude = std::abs(90.0 - 180.0 * (y + 0.5) / rows);
                        if (forcing.landMask[cell] || forcing.bathymetryMetres[cell] <= 0.0f ||
                            (band == 1 && latitude >= 20.0) ||
                            (band == 2 && (latitude < 20.0 || latitude >= 60.0)) ||
                            (band == 3 && latitude < 60.0)) continue;
                        const double weight = climategrid::latitudeBandMeasure(y, rows);
                        const auto& budget = ocean.heatBudget[cell];
                        const std::array<double, 11> values = {
                            forcing.surfaceHeatFluxReferenceTemperatureC[cell], forcing.surfaceHeatFluxWm2[cell],
                            surfaceRadiation[season][cell], surfaceLatentLoss[season][cell], surfaceSensibleLoss[season][cell],
                            ocean.meanSstC[cell], ocean.meanSurfaceSkinTemperatureC[cell], budget.surfaceWm2,
                            budget.horizontalWm2 + budget.verticalWm2, budget.storageWm2, forcing.atmosphericTemperatureC[cell]};
                        for (std::size_t k = 0; k < sums.size(); ++k) sums[k] += weight * values[k];
                        maximumClosure = std::max(maximumClosure, std::abs(values[1] - values[2] + values[3] + values[4]));
                        area += weight; ++cells;
                    }
                if (area == 0.0) continue;
                constexpr const char* labels[] = {"reference_c", "reference_flux_wm2", "radiation_wm2", "latent_loss_wm2",
                    "sensible_loss_wm2", "sst_c", "skin_c", "applied_surface_wm2", "net_transport_wm2", "storage_wm2", "air_c"};
                std::ostringstream trace;
                trace << std::setprecision(10) << "Ocean applied season=" << season << " band=" << band << " cells=" << cells;
                for (std::size_t k = 0; k < sums.size(); ++k) trace << ' ' << labels[k] << '=' << sums[k] / area;
                trace << " maximum_forcing_closure_wm2=" << maximumClosure;
                trace << " zonal_diffusion=" << (config.implicitZonalHeatDiffusion ? "implicit-v1" : "explicit-v1")
                    << " heat_substeps=" << ocean.heatSubsteps << " explicit_heat_substeps=" << ocean.explicitHeatSubsteps
                    << " zonal_iterations=" << ocean.maximumZonalDiffusionIterations
                    << " zonal_residual_k=" << ocean.maximumZonalDiffusionResidualK;
                std::cout << trace.str() << '\n';
            }
        const auto remapoutput = [&](const std::vector<float>& field)
        {
            return climategrid::remapField(
                outputRemap, field, columns);
        };
        ...
        // These are temperatures/cover over water, not extensive whole-cell
        // quantities. Land placeholders must not cool or de-ice coastal water.
        // Small seas with no resolved wet donor retain the base thermal state.
        std::vector<float> fallbackSst(outputcellcount), fallbackSkin(outputcellcount), fallbackIce(outputcellcount);
        for (int y = 0; y < outputrows; ++y)
            for (int x = 0; x < outputcolumns; ++x)
            {
                const auto cell = y * outputcolumns + x;
                const float base = static_cast<float>(world.seasonaltemp(season, x, y));
                fallbackSst[cell] = std::max(config.freezingTemperatureC, base);
                fallbackSkin[cell] = base;
                fallbackIce[cell] = base < config.freezingTemperatureC ? 1.0f : 0.0f;
            }
        auto sst = climateocean::detail::remapOceanThermalField(
            outputRemap, forcing.landMask, ocean.meanSstC, fallbackSst);
        ...
        const auto skin = climateocean::detail::remapOceanThermalField(
            outputRemap, forcing.landMask, ocean.meanSurfaceSkinTemperatureC, fallbackSkin);
        const auto ice = climateocean::detail::remapOceanThermalField(
            outputRemap, forcing.landMask, ocean.meanIceCover, fallbackIce);
        climate.hasOcean = true;
        climate.oceanSst[season].assign(outputcolumns, vector<float>(outputrows));
        climate.oceanSkin[season].assign(outputcolumns, vector<float>(outputrows));
        climate.oceanIce[season].assign(outputcolumns, vector<float>(outputrows));
        for (int y = 0; y < outputrows; ++y)
            for (int x = 0; x < outputcolumns; ++x)
            {
                const auto cell = y * outputcolumns + x;
                climate.oceanSst[season][x][y] = sst[cell];
                climate.oceanSkin[season][x][y] = skin[cell];
                climate.oceanIce[season][x][y] = ice[cell];
            }
        for (int y = 0; y < outputrows; y++)
        {
            for (int x = 0; x < outputcolumns; x++)
            {
                ...
                // This is a one-way ocean solve. Its coarser forcing is never
                // written back over the accepted atmospheric wind/pressure.
            }
        }
        if (isworldgendebugrunactive() && climate.winds.populated[season])
            climatevalidation::capturecirculationwindfields(world, season,
                climate.winds.surfacewindu[season], climate.winds.surfacewindv[season],
                climate.winds.upperwindu[season], climate.winds.upperwindv[season]);
        std::cout
    ...
    if (!tuning::climate::oceancurrents::oneWayDiagnosticsOnly)
        for (int y = 0; y < outputrows; ++y)
            for (int x = 0; x < outputcolumns; ++x)
            {
                int frozenSeasons = 0;
                for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
                    frozenSeasons += climate.oceanIce[season][x][y] >= 0.5f;
...
float pressuresurfacetemperature(planet& world, int season, int x, int y)
{
    ...
    if (world.sea(x, y) == 1 && climate.hasOcean &&
        climate.oceanSst[season].size() == static_cast<std::size_t>(world.width() + 1))
        return climate.oceanSkin[season][x][y];
...
void updatehorsebeltsfrompressure(planet& world)
{
    ...
    auto rowfromlatitude = [height](float latitude)
    {
        ...
        return std::clamp(static_cast<int>(std::round(
            (90.0f - latitude) * static_cast<float>(rows) / 180.0f - 0.5f)),
            0, height);
    };
...
void convergeclimatecoupling(planet& world, vector<vector<int>>& fractal)
{
    ...
    const int iterationLimit = climate.couplingIterationLimit > 0
        ? climate.couplingIterationLimit : maximumCouplingIterations;
    if (!climate.hasProcesses) return;
    const auto snapshot = [&]()
    {
        ...
        for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
        {
            ...
            for (int y = 0; y < fields.rows; ++y)
                for (int x = 0; x < fields.columns; ++x)
                {
                    ...
                    const int yy = std::min(world.height(), static_cast<int>((y + 0.5) * (world.height() + 1) / fields.rows));
                    const double weight = std::sqrt(std::max(0.0, std::cos(3.141592653589793 * ((y + 0.5) / fields.rows - 0.5))));
                    values[0].push_back(weight * climate.winds.surfacewindu[season][xx][yy]);
                    values[0].push_back(weight * climate.winds.surfacewindv[season][xx][yy]);
                    values[0].push_back(weight * climate.winds.upperwindu[season][xx][yy]);
                    values[0].push_back(weight * climate.winds.upperwindv[season][xx][yy]);
    ...
    auto old = snapshot();
    auto previousforcing = climate.processes;
    ...
    for (int iteration = 1; iteration <= iterationLimit; ++iteration)
    {
        if (iteration > 1)
        {
            ...
            for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
            {
                auto& fields = climate.processes[season];
                ...
                blend(fields.surfaceRadiativeHeatingWm2, previousforcing[season].surfaceRadiativeHeatingWm2);
                blend(fields.surfaceLatentHeatLossWm2, previousforcing[season].surfaceLatentHeatLossWm2);
                blend(fields.surfaceNetHeatingWm2, previousforcing[season].surfaceNetHeatingWm2);
                blend(fields.surfaceTemperatureC, previousforcing[season].surfaceTemperatureC);
            }
        }
        previousforcing = climate.processes;
        ...
        const int forcingColumns = std::max(1, climate.processes[0].columns);
        const int forcingRows = std::max(1, climate.processes[0].rows);
        const std::size_t peakCell = peakHeatingIndex / 3;
        d.innerSolvesAccepted = std::all_of(climate.oceanAccepted.begin(), climate.oceanAccepted.end(), [](bool accepted) { return accepted; }) &&
            std::all_of(climate.atmosphereAccepted.begin(), climate.atmosphereAccepted.end(), [](bool accepted) { return accepted; }) &&
            climatephysics::lastHydrologySpinupDiagnostics().converged && climate.oceanStorage.periodic;
    ...
    const auto& diagnostics = climatephysics::lastClimateCouplingDiagnostics();
    if (!diagnostics.empty() && !diagnostics.back().converged)
        std::cout << "Climate coupling did not converge within " << iterationLimit
            << " iterations; inspect the climate_coupling diagnostics.\n";
}
...
void createvectorwindmap(planet& world)
{
    ...
    const int columns = climatehydrology::climateGridDimensions(width + 1, height + 1,
        climate.resolution.atmosphereColumns).columns;
    ...
    const auto input = climategrid::makeConservativeRemap(
        width + 1, height + 1, columns, rows);
    ...
    {
        const auto flat = climategrid::remapSmoothField(
            columns, rows, field, width + 1, height + 1);
    ...
    vector<float> vegetationcontrast((width + 1) * (height + 1), 1.0f);
    if (atmosphere::vegetationDragContrastFraction > 0.0f)
    {
        const char* path = std::getenv("UW_CLIMATE_SURFACE_DRAG_INPUT");
        if (path && *path)
        {
            std::ifstream stream(path);
            vegetationcontrast = climateio::readSurfaceDragContrast(stream, width + 1, height + 1);
            std::cout << "Prescribed surface drag input=" << path << " strength="
                << atmosphere::vegetationDragContrastFraction << '\n';
        }
        else std::cout << "No prescribed vegetation input; retaining uniform land drag.\n";
    }
    auto nativevegetation = sample([&](int x, int y) {
        return world.nom(x,y) > world.sealevel() ? vegetationcontrast[y * (width + 1) + x] : 0.0f;
    });
    for (std::size_t cell = 0; cell < nativevegetation.size(); ++cell)
        nativevegetation[cell] = land[cell] > 0 ? nativevegetation[cell] / land[cell] : 1.0f;
    const auto terrain = sample([&](int x, int y) { return std::max(0, world.nom(x, y) - world.sealevel()); });
    const auto slopes = climateatmosphere::detail::terrainSlopes(grid, terrain,
        atmosphere::katabaticReflectedPolarGradient);
    const auto permanentice = sample([&](int x, int y) { return worldpermanentlandicefraction(world,x,y); });
    vector<float> persistentsnow(columns * rows, 0.0f);
    vector<float> persistentsnowoutput((width + 1) * (height + 1), 0.0f);
    if (atmosphere::persistentSnowDragFraction > 0.0f && climate.hasProcesses)
    {
        const auto& first = climate.processes[0];
        std::array<const vector<float>*, CLIMATESEASONCOUNT> quarters{};
        for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
        {
            const auto& fields = climate.processes[season];
            if (fields.durationSeconds > 0.0 && fields.columns == first.columns && fields.rows == first.rows)
                quarters[season] = &fields.landSnowCoverFraction;
        }
        if (first.columns > 0 && first.rows > 0)
        {
            const auto cover = climateatmosphere::detail::persistentSnowCover(
                quarters, static_cast<std::size_t>(first.columns) * first.rows);
            persistentsnow = climategrid::remapField(first.columns, first.rows, cover, columns, rows);
            persistentsnowoutput = climategrid::remapSmoothField(first.columns, first.rows, cover, width + 1, height + 1);
        }
    }
    // Compare the reduced pressure mode's thermal columns at a common level.
    // This removes only the lapse offset imposed by the temperature module;
    // actual temperatures used for energy, snow and cold drainage are retained.
    const auto pressurelapseoffset = sample([&](int x, int y)
    {
        return world.sea(x,y) ? 0.0f : circulation::thermalPressureElevationCorrectionFraction *
            world.tempdecrease() * std::max(0, world.map(x,y) - world.sealevel()) / 1000.0f;
    });
    const auto terrainoutput = output(terrain);
    const auto landoutput = output(land);
    // Preserve resolved world ridges for moisture uplift; the pressure solver
    // still uses its own conservative atmosphere-grid terrain above.
    floatgrid orographicterrain(width + 1, vector<float>(height + 1));
    for (int y = 0; y <= height; ++y)
        for (int x = 0; x <= width; ++x)
            orographicterrain[x][y] = static_cast<float>(std::max(0, world.nom(x, y) - world.sealevel()));
    const auto surfacedrag = [](float landFraction, float elevationMetres, float frozenLandFraction, float vegetationContrast)
    {
        return climateatmosphere::detail::vegetationSurfaceDragCoefficient(landFraction, elevationMetres,
            atmosphere::frozenSurfaceDragFraction * frozenLandFraction, atmosphere::oceanMomentumDragCoefficient,
            atmosphere::landMomentumDragCoefficient, atmosphere::highReliefMomentumDragCoefficient,
            atmosphere::frozenMomentumDragCoefficient, vegetationContrast, atmosphere::vegetationDragContrastFraction);
    };
    vector<float> parallelDrag;
    if (circulation::surfaceDragParallelMobility)
    {
        vector<float> localDrag((width + 1) * (height + 1));
        for (int y = 0; y <= height; ++y)
            for (int x = 0; x <= width; ++x)
            {
                const int cell = y * (width + 1) + x;
                localDrag[cell] = surfacedrag(world.nom(x, y) > world.sealevel() ? 1.0f : 0.0f,
                    static_cast<float>(std::max(0, world.nom(x, y) - world.sealevel())),
                    std::max(worldpermanentlandicefraction(world, x, y),
                        atmosphere::persistentSnowDragFraction * persistentsnowoutput[cell]), vegetationcontrast[cell]);
            }
        parallelDrag = climateatmosphere::detail::parallelSurfaceDragCoefficients(
            width + 1, height + 1, localDrag, columns, rows);
    }
    climate.winds.reset(world.width(), world.height());
    for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
    {
        ...
        // A stationary climate response needs a fixed seasonal background.
        // Re-selecting a warmest SST row on every coupling pass moves the entire
        // Hadley cell discontinuously when neighbouring warm rows exchange rank.
        // Local thermal pressure gradients below still respond to evolving SST.
        auto& radiative = climate.radiativeZonalTemperature[season];
        if (radiative.size() != rowtemperature.size()) radiative = rowtemperature;
        const auto& background = climate.transientWeather ? rowtemperature : radiative;
        double backgroundMean = 0.0;
        for (int y = 0; y < rows; ++y) backgroundMean += grid.cellAreasSquareMetres[y] * background[y] / totalarea;
        const int warmrow = static_cast<int>(std::max_element(background.begin(), background.end()) - background.begin());
        const float contrast = std::max(1.0f, background[warmrow] - 0.5f * (background.front() + background.back()));
        climateatmosphere::ModeSeparatedCirculationConfig config;
        config.adjustZonalSurfacePressure = circulation::adjustZonalSurfacePressure;
        config.adjustZonalUpperHeight = circulation::adjustZonalUpperHeight;
        config.boundaryLayer.enabled = climate.mixedLayerTropics;
        ...
        config.interlayerMomentumCoupling = circulation::stationaryInterlayerMomentumCoupling;
        ...
        config.stationarySolver = climate.atmosphereSolver;
        config.stationaryCaptureDirectory = climate.atmosphereCaptureDirectory;
        ...
        const float hadleywidth = std::clamp(climateatmosphere::heldHouHadleyEdgeLatitudeDegrees(
            contrast, circulation::troposphereHeightMetres, static_cast<float>(backgroundMean + 273.15),
        ...
        const float confinedshift = hadleywidth * circulation::confinedThermalEquatorShiftHadleyFraction;
        const float confinedtrough = std::clamp(static_cast<float>(grid.latitudeCentresRadians[warmrow] * 180.0 / 3.141592653589793),
            -confinedshift, confinedshift);
        ...
        const auto upperthermal = climateatmosphere::detail::projectUpperThermalGradient(
            background, grid.cellAreasSquareMetres, circulation::upperThermalCoreGradientFraction,
            1.0f, 0.7f * hadleywidth, 1.7f * hadleywidth,
            circulation::upperThermalPolarGradientFraction,
            circulation::upperThermalPolarStartLatitude, circulation::upperThermalPolarEndLatitude);
        double pressuremean = 0.0;
        vector<float> continentalPressure(columns * rows, 0.0f);
        for (int y = 0; y < rows; ++y)
        {
            zonalpressure[y] = climateatmosphere::detail::confinedOverturningPressureHpa(
                static_cast<float>(grid.latitudeCentresRadians[y] * 180.0 / 3.141592653589793),
                thermalequator, hadleywidth, circulation::winterHadleyEdgeAnchorFraction, confinedtrough, amplitude);
            const float localcontrast = std::max(1.0f, background[warmrow] -
                (grid.latitudeCentresRadians[y] >= 0 ? background.front() : background.back()));
            zonalpressure[y] = climateatmosphere::detail::polarBranchPressureHpa(zonalpressure[y],
                static_cast<float>(grid.latitudeCentresRadians[y] * 180.0 / 3.141592653589793),
                thermalequator, hadleywidth, circulation::winterHadleyEdgeAnchorFraction,
                amplitude, localcontrast / contrast, circulation::polarThermalContrastFraction);
            zonalpressure[y] = climateatmosphere::detail::deepenSubpolarLowPressureHpa(zonalpressure[y],
                static_cast<float>(grid.latitudeCentresRadians[y] * 180.0 / 3.141592653589793),
                thermalequator, hadleywidth, circulation::winterHadleyEdgeAnchorFraction,
                amplitude, circulation::subpolarLowDeepeningFraction,
                circulation::subpolarDeepeningPreservePolarGradient);
            zonalpressure[y] = climateatmosphere::detail::hemisphericSubpolarPressureHpa(
                zonalpressure[y], static_cast<float>(grid.latitudeCentresRadians[y] * 180.0 / 3.141592653589793),
                thermalequator, hadleywidth, circulation::winterHadleyEdgeAnchorFraction,
                localcontrast / contrast, circulation::subpolarHemisphericContrastExponent);
            double continentalMean = 0.0;
            for (int x = 0; x < columns; ++x)
            {
                const int cell = y * columns + x;
                continentalPressure[cell] = climateatmosphere::detail::continentalSubpolarCorrectionHpa(
                    zonalpressure[y], static_cast<float>(grid.latitudeCentresRadians[y] * 180.0 / 3.141592653589793),
                    thermalequator, hadleywidth, circulation::winterHadleyEdgeAnchorFraction,
                    land[cell], circulation::continentalSubpolarLowSuppression);
                continentalMean += continentalPressure[cell] / columns;
            }
            // Carry the row mean in the zonal solve and the remainder in the
            // stationary solve. The global gauge below still conserves mass.
            zonalpressure[y] += static_cast<float>(continentalMean);
            for (int x = 0; x < columns; ++x)
                continentalPressure[y * columns + x] -= static_cast<float>(continentalMean);
            pressuremean += grid.cellAreasSquareMetres[y] * zonalpressure[y] / totalarea;
            config.zonalUpperHeightMetres[y] = hydrostatic * upperthermal[y];
        }
        ...
        {
            const float drag = parallelDrag.empty() ? surfacedrag(land[cell], terrain[cell], std::max(permanentice[cell],
                atmosphere::persistentSnowDragFraction * persistentsnow[cell]), nativevegetation[cell]) : parallelDrag[cell];
        ...
        const auto mechanicalforcing = surfaceforcing;
        auto thermaltemperature = temperature;
        if (circulation::surfaceThermalLandSkinFraction > 0.0f && climate.hasProcesses)
        {
            const auto& fields = climate.processes[season];
            if (fields.columns > 0 && fields.rows > 0 && fields.surfaceTemperatureC.size() ==
                static_cast<std::size_t>(fields.columns) * fields.rows)
            {
                const auto skin = climategrid::remapField(fields.columns, fields.rows,
                    fields.surfaceTemperatureC, columns, rows);
                const float fraction = std::clamp(circulation::surfaceThermalLandSkinFraction, 0.0f, 1.0f);
                for (int cell = 0; cell < columns * rows; ++cell)
                    if (std::isfinite(skin[cell]))
                        thermaltemperature[cell] += fraction * land[cell] * (skin[cell] - temperature[cell]);
            }
        }
        vector<float> thermalrowtemperature(rows, 0.0f);
        for (int cell = 0; cell < columns * rows; ++cell)
        {
            thermaltemperature[cell] += pressurelapseoffset[cell];
            thermalrowtemperature[cell / columns] += thermaltemperature[cell] / columns;
        }
        ...
        for (int y = 0; y < rows; ++y)
            ...
            {
                const int cell = y * columns + x;
                const float anomaly = thermaltemperature[cell] - thermalrowtemperature[y];
                // Blend integrated hydrostatic responses, not top pressures.
                // This retains the ocean response exactly and mixes coastline
                // tiles continuously as the continental column is deepened.
                surfaceforcing[cell] += continentalPressure[cell] +
                    (1.0f - land[cell]) * climateatmosphere::thermalModePressureAnomalyHpa(
                        circulation::surfaceThermalProfileFraction * anomaly, config.airDensityKgM3,
                        circulation::surfaceReferencePressurePa, circulation::surfaceThermalTopPressurePa) +
                    land[cell] * climateatmosphere::thermalModePressureAnomalyHpa(
                        circulation::surfaceThermalLandProfileFraction * anomaly, config.airDensityKgM3,
                        circulation::surfaceReferencePressurePa, circulation::surfaceThermalLandTopPressurePa);
            }
        vector<float> regionalPressure(columns * rows, 0.0f), regionalTrough(columns * rows, confinedtrough);
        if (circulation::regionalThermalTroughFraction > 0.0f)
        {
            const auto targets = climateatmosphere::detail::regionalThermalEquatorDegrees(
                columns, rows, thermaltemperature, grid.latitudeCentresRadians, confinedtrough, hadleywidth);
            for (int y = 0; y < rows; ++y)
            {
                double rowMean = 0.0;
                const float latitude = static_cast<float>(grid.latitudeCentresRadians[y] * 180.0 / 3.141592653589793);
                for (int x = 0; x < columns; ++x)
                {
                    const int cell = y * columns + x;
                    const float target = confinedtrough + circulation::regionalThermalTroughFraction * (targets[x] - confinedtrough);
                    regionalTrough[cell] = target;
                    const auto pressureAt = [&](float trough) {
                        return climateatmosphere::detail::confinedOverturningPressureHpa(latitude,
                            thermalequator, hadleywidth, circulation::winterHadleyEdgeAnchorFraction, trough, amplitude);
                    };
                    regionalPressure[cell] = pressureAt(target) - pressureAt(confinedtrough);
                    rowMean += regionalPressure[cell] / columns;
                }
                // This is a regional departure. The zonal pressure template and
                // its subtropical edges are not translated by local heating.
                for (int x = 0; x < columns; ++x)
                {
                    const int cell = y * columns + x;
                    regionalPressure[cell] -= static_cast<float>(rowMean);
                    surfaceforcing[cell] += regionalPressure[cell];
                }
            }
        }
        auto upperWaveTerrain = terrain;
        if (circulation::upperOrographicBlocking)
            for (int y = 0; y < rows; ++y)
            {
                float backgroundWind = 0.0f;
                for (int x = 0; x < columns; ++x)
                    backgroundWind += zonal.upperEastWindMps[y * columns + x] / columns;
                for (int x = 0; x < columns; ++x)
                {
                    const int cell = y * columns + x;
                    upperWaveTerrain[cell] = circulation::upperOrographicLaunchFraction *
                        climateatmosphere::detail::effectiveMountainWaveHeightMetres(
                        terrain[cell], backgroundWind, stability);
                }
            }
        config.upperOrographicHeightMetres = climateatmosphere::upperOrographicHeightForcing(columns, rows,
            upperWaveTerrain, zonal.upperEastWindMps, stability, hydrostatic * static_cast<float>(globaltemperature + 273.15),
            config.upperDampingTimeSeconds, config);
        constexpr std::array<float, CLIMATESEASONCOUNT> days = {15.0f, 105.0f, 196.0f, 288.0f};
        const auto solar = climateenergy::solarForcing(days[season], world.tilt(), world.eccentricity(), world.perihelion());
        const float declination = solar.declinationRadians, distance = solar.distanceFactor;
            ...
            for (int layer = 0; layer < 2; ++layer)
            {
                ...
                for (std::size_t cell = 0; cell < net.size(); ++cell)
                    net[cell] = fields.radiativeHeatingWm2[layer][cell] +
                        ...
                        (layer == 0 ? fields.sensibleHeatingWm2[cell] : 0.0f);
                (layer == 0 ? lowerheating : upperheating) = climategrid::remapField(
                    fields.columns, fields.rows, net, columns, rows);
            }
        }
        ...
        // The fixed 5 m/s pressure linearization overestimates stress in calm
        // forest cells. Update its positive drag rate from the diagnosed flow;
        // retain the full receiver roughness and the existing mass-flux closure.
        if (!climate.transientWeather && !config.boundaryLayer.enabled)
            for (int iteration = 0; iteration < circulation::surfacePressureDragIterations; ++iteration)
            {
                if (!result.surfaceStationarySolver.converged) break;
                double maximumChange = 0.0;
                for (int cell = 0; cell < columns * rows; ++cell)
                {
                    const double speed = std::max(circulation::surfacePressureDragMinimumWindMps,
                        std::hypot(result.surfaceEastWindMps[cell], result.surfaceSouthWindMps[cell]));
                    const double target = config.surfaceDragCoefficients[cell] * speed /
                        config.surfaceBoundaryLayerDepthMetres;
                    const double old = 1.0 / config.surfaceDragTimesSeconds[cell];
                    const double updated = old + circulation::surfacePressureDragRelaxation * (target - old);
                    maximumChange = std::max(maximumChange, std::abs(target - old) / std::max(target, old));
                    config.surfaceDragTimesSeconds[cell] = static_cast<float>(1.0 / updated);
                }
                result = climateatmosphere::solveModeSeparatedCirculation(columns, rows, zonalpressure,
                    lowerheating, surfaceforcing, config);
                std::cout << "Surface nonlinear drag season=" << season << " iteration=" << iteration
                    << " maximum_relative_rate_change=" << maximumChange << '\n';
            }
        climate.atmosphereAccepted[season] = result.surfaceStationarySolver.converged && result.upperStationarySolver.converged;
        if (climate.transientWeather && circulation::enablePrognosticClimateAtmosphere && config.enabled.stationary &&
        ...
        if (climate.mixedLayerTropics && config.enabled.surface)
        {
            // Calibrated projection into the reduced inversion mode. Applying
            // the former surface amplitude unchanged overdrives 850 hPa winds.
            constexpr float inversionOverturningProjection = 0.4f;
            config.inversionEquilibriumPressureHpa.resize(columns * rows);
            for (int y = 0; y < rows; ++y)
                for (int x = 0; x < columns; ++x)
                    config.inversionEquilibriumPressureHpa[y * columns + x] = config.enabled.zonal ?
                        inversionOverturningProjection * zonalpressure[y] : 0.0f;
            vector<float> columnheating(columns * rows);
            for (std::size_t i = 0; i < columnheating.size(); ++i) columnheating[i] = lowerheating[i] + upperheating[i];
            auto& layer = climate.boundaryLayer[season];
            layer = climateatmosphere::solveBoundaryLayer(columns, rows, temperature, columnheating, mechanicalforcing, config);
            const bool accepted = climateatmosphere::applyBoundaryLayer(layer, config, result);
            climate.atmosphereAccepted[season] = climate.atmosphereAccepted[season] && accepted;
            std::cout << "Tropical boundary layer season=" << season << " accepted=" << accepted
                << " inversion_hpa=" << config.boundaryLayer.inversionPressurePa / 100.0f
                << " overturning_projection=" << inversionOverturningProjection
                << " residual=" << layer.inversionSolver.relativeResidual
                << " iterations=" << layer.inversionSolver.iterations
                << " momentum_residual_mps2=" << result.maximumBoundaryLayerMomentumResidualMps2 << '\n';
        }
        const auto nativeSolvedPressure = result.surfacePressureAnomalyHpa;
        if (const auto* referencepressure = referenceexperiment().field(columns, "slp"))
        {
            for (int y = 0; y < rows; ++y)
                for (int x = 0; x < columns; ++x)
                    result.surfacePressureAnomalyHpa[y * columns + x] = climateio::experiment::sample(
                        referencepressure, season, season, 0.0f, x, y, result.surfacePressureAnomalyHpa[y * columns + x]);
            // Replace the diagnosed pressure, not its equilibrium forcing. This also
            // recomputes ascent from the resulting pressure fluxes before later corrections.
            climateatmosphere::diagnoseModeWinds(columns, rows, config, result);
            std::cout << "Reference surface pressure imposed at wind diagnosis season=" << season << '\n';
        }
        const auto preAdvectionEast = result.surfaceEastWindMps, preAdvectionSouth = result.surfaceSouthWindMps;
        if (circulation::surfaceMomentumAdvectionIterations > 0 && !climate.transientWeather &&
            !config.boundaryLayer.enabled && config.enabled.surface)
        {
            // Recover the existing effective forcing, including stationary
            // exchange, before adding bulk horizontal momentum transport.
            // Pressure remains the preceding inversion; this is a bounded
            // stationary correction, not a prognostic pressure closure.
            vector<float> forceEast(columns * rows), forceNorth(columns * rows);
            for (int y = 0; y < rows; ++y)
                for (int x = 0; x < columns; ++x)
                {
                    const int cell = y * columns + x;
                    const auto force = climateatmosphere::detail::surfaceMomentumForce(
                        {preAdvectionEast[cell], preAdvectionSouth[cell]}, config.surfaceDragCoefficients[cell],
                        latitudeforrow(y, rows - 1), config);
                    forceEast[cell] = force.eastMps2; forceNorth[cell] = force.northMps2;
                }
            const auto advected = climateatmosphere::detail::advectSurfaceMomentum(grid, forceEast, forceNorth,
                config.surfaceDragCoefficients, preAdvectionEast, preAdvectionSouth, config,
                circulation::surfaceMomentumAdvectionIterations);
            if (advected.east.size() == preAdvectionEast.size())
            {
                vector<float> du(columns * rows), dv(columns * rows);
                for (int cell = 0; cell < columns * rows; ++cell)
                {
                    du[cell] = advected.east[cell] - preAdvectionEast[cell];
                    dv[cell] = advected.south[cell] - preAdvectionSouth[cell];
                }
                const auto ascentchange = climateatmosphere::detail::surfaceAscentCorrectionHpaPerDay(
                    grid, du, dv, 0.5f * config.surfaceEquivalentPressureDepthHpa);
                for (std::size_t cell = 0; cell < ascentchange.size(); ++cell) result.ascentHpaPerDay[cell] += ascentchange[cell];
                result.surfaceEastWindMps = advected.east; result.surfaceSouthWindMps = advected.south;
                std::cout << "Surface momentum advection season=" << season << " iterations=" << advected.iterations
                    << " maximum_change_mps=" << advected.maximumChangeMps
                    << " relative_momentum_residual=" << advected.relativeMomentumResidual
                    << " forcing_work_wm2=" << advected.forcingWorkWm2 << " drag_work_wm2=" << advected.dragWorkWm2
                    << " advection_work_wm2=" << advected.advectionWorkWm2
                    << " upwind_mixing_work_wm2=" << advected.upwindMixingWorkWm2
                    << " ascent_integral=" << climategrid::areaWeightedIntegral(columns, rows, ascentchange) << '\n';
            }
        }
        vector<float> wind850East(columns * rows, 0.0f), wind850South(columns * rows, 0.0f),
            pressure850(columns * rows, 0.0f), available850(columns * rows, 0.0f), thermal850(columns * rows, 0.0f);
        const auto pre850SurfaceEast = result.surfaceEastWindMps, pre850SurfaceSouth = result.surfaceSouthWindMps;
        auto preExchange850East = wind850East, preExchange850South = wind850South;
        auto native850East = wind850East, native850South = wind850South;
        if (circulation::enableAuxiliary850Momentum && !climate.transientWeather &&
            !config.boundaryLayer.enabled && config.enabled.surface)
        {
            // Diagnose a distinct lower-free-tropospheric mode without applying
            // the alternative surface closure or replacing the 500 hPa mode.
            auto lowConfig = config;
            // Keep inversion mass adjustment independent of the surface mode.
            // Sharing its equivalent depth made a shallow surface-pressure
            // experiment also amplify the weakly damped 850 hPa circulation.
            const auto lowParameters = climateatmosphere::diagnoseStationaryParameters(stability,
                circulation::auxiliary850ModeDepthMetres, config.gravityMetresPerSecondSquared,
                config.planetRadiusMetres, config.rotationRatePerSecond, columns, rows,
                circulation::stationaryWaveNondimensionalDamping, circulation::diagnosedForcingScaleMetres);
            lowConfig.surfaceEquivalentPressureDepthHpa = pressurepermetre * lowParameters.equivalentDepthMetres;
            lowConfig.surfaceDampingTimeSeconds = lowParameters.dampingTimeSeconds;
            lowConfig.boundaryLayer.enabled = true;
            lowConfig.stationaryCaptureDirectory.clear();
            lowConfig.inversionEquilibriumPressureHpa.resize(columns * rows);
            vector<float> columnHeating(columns * rows);
            for (int cell = 0; cell < columns * rows; ++cell)
            {
                // Reduced-mode sensitivity only: actual column energy remains unchanged.
                columnHeating[cell] = circulation::auxiliary850HeatingResponseFraction *
                    (lowerheating[cell] + upperheating[cell]);
                // Stored regional warmth is distinct from instantaneous net
                // heating. Approximate an anomaly decaying toward 500 hPa;
                // use the same pressure-only elevation proxy as the surface.
                thermal850[cell] = climateatmosphere::thermalModePressureAnomalyHpa(
                    circulation::auxiliary850ThermalProfileFraction * (thermaltemperature[cell] - thermalrowtemperature[cell / columns]),
                    lowConfig.boundaryLayer.inversionDensityKgM3, lowConfig.boundaryLayer.inversionPressurePa, 50000.0f);
                lowConfig.inversionEquilibriumPressureHpa[cell] = circulation::auxiliary850OverturningProjection *
                    (circulation::auxiliary850ZonalPressureFraction * zonalpressure[cell / columns] +
                        regionalPressure[cell] + continentalPressure[cell]) + thermal850[cell];
            }
            const auto low = climateatmosphere::solveBoundaryLayer(columns, rows, temperature,
                columnHeating, mechanicalforcing, lowConfig);
            if (low.inversionSolver.converged)
            {
                pressure850 = low.inversionPressureHpa;
                for (int cell = 0; cell < columns * rows; ++cell)
                {
                    const float height850 = 287.05f * (temperature[cell] + 273.15f) /
                        config.gravityMetresPerSecondSquared * std::log(100000.0f / lowConfig.boundaryLayer.inversionPressurePa);
                    // Approximate pressure-surface availability, not cut-cell terrain dynamics.
                    available850[cell] = terrain[cell] < height850 ? 1.0f : 0.0f;
                    wind850East[cell] = available850[cell] * low.inversionEastWindMps[cell];
                    wind850South[cell] = available850[cell] * low.inversionSouthWindMps[cell];
                }
                native850East = wind850East; native850South = wind850South;
                const auto* reference850east = referenceexperiment().field(columns, "u850");
                const auto* reference850north = referenceexperiment().field(columns, "v850");
                if (bool(reference850east) != bool(reference850north))
                    throw std::runtime_error("Reference 850-hPa wind requires both components");
                if (reference850east)
                {
                    for (int y = 0; y < rows; ++y)
                        for (int x = 0; x < columns; ++x)
                        {
                            const int cell = y * columns + x;
                            if (available850[cell] == 0.0f) continue;
                            wind850East[cell] = climateio::experiment::sample(reference850east,
                                season, season, 0.0f, x, y, wind850East[cell]);
                            wind850South[cell] = -climateio::experiment::sample(reference850north,
                                season, season, 0.0f, x, y, -wind850South[cell]);
                        }
                    std::cout << "Reference 850-hPa driver imposed before two-way exchange season=" << season << '\n';
                }
                preExchange850East = wind850East; preExchange850South = wind850South;
                const float surfaceMass = config.airDensityKgM3 * config.surfaceBoundaryLayerDepthMetres;
                // The remainder of the nominal surface-to-850 pressure mass is
                // a bulk low-level reservoir represented by the 850 hPa wind.
                // This is not an additional prognostic moisture-transport layer.
                const float lowMass = (100000.0f - lowConfig.boundaryLayer.inversionPressurePa) /
                    config.gravityMetresPerSecondSquared - surfaceMass;
                if (circulation::surface850ExchangeRatePerSecond > 0.0f && lowMass > 0.0f)
                {
                    vector<float> du(columns * rows, 0.0f), dv(columns * rows, 0.0f);
                    double area = 0.0, loss = 0.0, surfaceWork = 0.0, lowWork = 0.0, maximumResidual = 0.0;
                    int failed = 0;
                    const float rayleigh = 1.0f / lowConfig.boundaryLayer.inversionDragTimeSeconds;
                    for (int y = 0; y < rows; ++y)
                        for (int x = 0; x < columns; ++x)
                        {
                            const int cell = y * columns + x;
                            ...
                            area += cellArea;
                            const float latitude = latitudeforrow(y, rows - 1);
                            const float exchange = circulation::surface850ExchangeRatePerSecond * available850[cell] *
                                climateatmosphere::boundaryLayerWeight(latitude, lowConfig.boundaryLayer);
                            if (exchange == 0.0f) continue;
                            const float f = climateatmosphere::coriolisParameterPerSecond(latitude,
                                config.rotationRatePerSecond, config.rotationDirection);
                            const auto surfaceForce = climateatmosphere::detail::surfaceMomentumForce(
                                {pre850SurfaceEast[cell], pre850SurfaceSouth[cell]}, config.surfaceDragCoefficients[cell], latitude, config);
                            const climateatmosphere::detail::SurfaceMomentumForce lowForce{
                                rayleigh * wind850East[cell] + f * wind850South[cell],
                                f * wind850East[cell] - rayleigh * wind850South[cell]};
                            const auto coupled = climateatmosphere::detail::coupleSurface850(surfaceForce, lowForce, f,
                                config.surfaceDragCoefficients[cell], config.surfaceBoundaryLayerDepthMetres,
                                config.airDensityKgM3, lowMass, rayleigh, exchange);
                            if (!coupled.valid) { ++failed; continue; }
                            result.surfaceEastWindMps[cell] = coupled.surface.eastMetresPerSecond;
                            result.surfaceSouthWindMps[cell] = coupled.surface.southMetresPerSecond;
                            wind850East[cell] = coupled.lowLevel.eastMetresPerSecond;
                            wind850South[cell] = coupled.lowLevel.southMetresPerSecond;
                            du[cell] = result.surfaceEastWindMps[cell] - pre850SurfaceEast[cell];
                            dv[cell] = result.surfaceSouthWindMps[cell] - pre850SurfaceSouth[cell];
                            loss += cellArea * coupled.mixingLossWm2;
                            surfaceWork += cellArea * (coupled.stressEastNm2 * result.surfaceEastWindMps[cell] -
                                coupled.stressNorthNm2 * result.surfaceSouthWindMps[cell]);
                            lowWork -= cellArea * (coupled.stressEastNm2 * wind850East[cell] -
                                coupled.stressNorthNm2 * wind850South[cell]);
                            maximumResidual = std::max(maximumResidual, coupled.maximumResidualMps2);
                        }
                    const auto ascentchange = climateatmosphere::detail::surfaceAscentCorrectionHpaPerDay(
                        grid, du, dv, 0.5f * config.surfaceEquivalentPressureDepthHpa);
                    for (std::size_t cell = 0; cell < ascentchange.size(); ++cell) result.ascentHpaPerDay[cell] += ascentchange[cell];
                    std::cout << "Surface 850 exchange season=" << season << " failed_cells=" << failed
                        << " low_mass_kgm2=" << lowMass << " maximum_momentum_residual_mps2=" << maximumResidual
                        << " mixing_loss_wm2=" << loss / area << " surface_work_wm2=" << surfaceWork / area
                        << " low_work_wm2=" << lowWork / area
                        << " work_balance_wm2=" << (loss + surfaceWork + lowWork) / area
                        << " ascent_integral=" << climategrid::areaWeightedIntegral(columns, rows, ascentchange) << '\n';
                }
            }
            std::cout << "Auxiliary 850 hPa season=" << season << " converged=" << low.inversionSolver.converged
                << " residual=" << low.inversionSolver.relativeResidual
                << " overturning_projection=" << circulation::auxiliary850OverturningProjection
                << " exchange_rate_per_s=" << circulation::surface850ExchangeRatePerSecond << '\n';
        }
        vector<float> carrierEast, carrierSouth;
        const auto preDrainageEast = result.surfaceEastWindMps, preDrainageSouth = result.surfaceSouthWindMps;
        auto diagnosticSkin = temperature, diagnosticSnow = permanentice;
        vector<float> diagnosticDeficit(columns * rows, 0.0f), diagnosticForceEast(columns * rows, 0.0f),
            diagnosticForceNorth(columns * rows, 0.0f);
        vector<float> diagnosticRadiativeDeficit(columns * rows, 0.0f), diagnosticRadiation(columns * rows, 0.0f);
        const float drainageMean = atmosphere::katabaticShallowTenMetreDiagnostic ?
            climateatmosphere::detail::coldJetTransportFraction(
                atmosphere::katabaticShallowDepthMetres, config.surfaceBoundaryLayerDepthMetres) :
            atmosphere::separateKatabaticTransport ?
            climateatmosphere::detail::katabaticLayerMeanFraction(
                atmosphere::katabaticVelocityDecayHeightMetres, config.surfaceBoundaryLayerDepthMetres) : 1.0f;
        climate.winds.hasDrainageProfile[season] = false;
        if (atmosphere::katabaticDeficitFraction > 0.0f && !climate.transientWeather &&
            !config.boundaryLayer.enabled && config.enabled.surface && climate.hasProcesses)
        {
            const auto started = std::chrono::steady_clock::now();
            const auto& fields = climate.processes[season];
            const auto skin = climategrid::remapField(fields.columns, fields.rows,
                fields.surfaceTemperatureC, columns, rows);
            const auto snow = climategrid::remapField(fields.columns, fields.rows,
                fields.landSnowCoverFraction, columns, rows);
            const auto surfaceRadiation = climategrid::remapField(fields.columns, fields.rows,
                fields.surfaceRadiativeHeatingWm2, columns, rows);
            diagnosticRadiation = surfaceRadiation;
            diagnosticSkin = skin;
            for (int cell = 0; cell < columns * rows; ++cell) diagnosticSnow[cell] = std::max(snow[cell], permanentice[cell]);
            vector<float> deltau(columns * rows), deltav(columns * rows);
            if (atmosphere::separateKatabaticTransport)
            {
                carrierEast = result.surfaceEastWindMps;
                carrierSouth = result.surfaceSouthWindMps;
            }
            double area = 0.0, activeArea = 0.0, deficitArea = 0.0, speedChangeArea = 0.0, powerArea = 0.0;
            float maximumForce = 0.0f, maximumDeficit = 0.0f;
            for (int y = 0; y < rows; ++y)
                for (int x = 0; x < columns; ++x)
                {
                    const int cell = y * columns + x;
                    const auto skinDeficitFunction = atmosphere::katabaticSnowPatchTemperature ?
                        climateatmosphere::detail::snowPatchKatabaticDeficitK : climateatmosphere::detail::katabaticDeficitK;
                    const float skinDeficit = skinDeficitFunction(
                        temperature[cell], skin[cell], std::max(snow[cell], permanentice[cell]), atmosphere::katabaticDeficitFraction,
                        atmosphere::katabaticMaximumDeficitK);
                    const float radiativeDeficit = climateatmosphere::detail::radiativeKatabaticDeficitK(
                        surfaceRadiation[cell], skin[cell], std::max(snow[cell], permanentice[cell]), config.airDensityKgM3,
                        1004.0f, atmosphere::katabaticShallowTenMetreDiagnostic ? atmosphere::katabaticShallowDepthMetres :
                            atmosphere::katabaticVelocityDecayHeightMetres, atmosphere::katabaticColdLayerRenewalSeconds,
                        atmosphere::katabaticRadiativeCoolingFraction, atmosphere::katabaticMaximumDeficitK);
                    // These are alternative estimates of the same cold anomaly.
                    const float deficit = std::max(skinDeficit, radiativeDeficit);
                    diagnosticRadiativeDeficit[cell] = radiativeDeficit;
                    const auto katabatic = climateatmosphere::detail::katabaticForce(
                        slopes[cell].east, slopes[cell].north, temperature[cell] + 273.15f,
                        deficit, config.gravityMetresPerSecondSquared, atmosphere::katabaticMaximumSlope);
                    const float magnitude = std::hypot(katabatic.eastMps2, katabatic.northMps2);
                    diagnosticDeficit[cell] = deficit;
                    diagnosticForceEast[cell] = katabatic.eastMps2;
                    diagnosticForceNorth[cell] = katabatic.northMps2;
                    area += grid.cellAreasSquareMetres[y];
                    if (magnitude == 0.0f) continue;
                    const climateatmosphere::HorizontalWind original{result.surfaceEastWindMps[cell], result.surfaceSouthWindMps[cell]};
                    const auto force = climateatmosphere::detail::surfaceMomentumForce(original,
                        config.surfaceDragCoefficients[cell], latitudeforrow(y, rows - 1), config);
                    const auto wind = atmosphere::katabaticShallowTenMetreDiagnostic ?
                        climateatmosphere::detail::coldJetTenMetreWind(original, katabatic, latitudeforrow(y, rows - 1),
                            config.surfaceDragCoefficients[cell] * atmosphere::katabaticTenMetreStressFraction,
                            atmosphere::katabaticShallowDepthMetres, config.rotationRatePerSecond, config.rotationDirection) :
                        climateatmosphere::steadyQuadraticDragCoriolisWind(
                        force.eastMps2 + katabatic.eastMps2, force.northMps2 + katabatic.northMps2,
                        latitudeforrow(y, rows - 1), config.surfaceDragCoefficients[cell],
                        config.surfaceBoundaryLayerDepthMetres, config.rotationRatePerSecond, config.rotationDirection);
                    deltau[cell] = wind.eastMetresPerSecond - original.eastMetresPerSecond;
                    deltav[cell] = wind.southMetresPerSecond - original.southMetresPerSecond;
                    result.surfaceEastWindMps[cell] = wind.eastMetresPerSecond;
                    result.surfaceSouthWindMps[cell] = wind.southMetresPerSecond;
                    activeArea += grid.cellAreasSquareMetres[y];
                    deficitArea += grid.cellAreasSquareMetres[y] * deficit;
                    speedChangeArea += grid.cellAreasSquareMetres[y] *
                        (std::hypot(wind.eastMetresPerSecond, wind.southMetresPerSecond) -
                         std::hypot(original.eastMetresPerSecond, original.southMetresPerSecond));
                    const float powerDepth = atmosphere::katabaticShallowTenMetreDiagnostic ?
                        atmosphere::katabaticShallowDepthMetres : config.surfaceBoundaryLayerDepthMetres;
                    const float powerMean = atmosphere::katabaticShallowTenMetreDiagnostic ?
                        static_cast<float>(climateatmosphere::detail::coldJetMeanToTenMetres(powerDepth)) : 1.0f;
                    powerArea += grid.cellAreasSquareMetres[y] * config.airDensityKgM3 * powerDepth *
                        (katabatic.eastMps2 * (original.eastMetresPerSecond + powerMean * deltau[cell]) -
                         katabatic.northMps2 * (original.southMetresPerSecond + powerMean * deltav[cell]));
                    maximumForce = std::max(maximumForce, magnitude);
                    maximumDeficit = std::max(maximumDeficit, deficit);
                }
            for (float& value : deltau) value *= drainageMean;
            for (float& value : deltav) value *= drainageMean;
            const auto ascentchange = climateatmosphere::detail::surfaceAscentCorrectionHpaPerDay(
                grid, deltau, deltav, 0.5f * config.surfaceEquivalentPressureDepthHpa);
            for (std::size_t cell = 0; cell < ascentchange.size(); ++cell) result.ascentHpaPerDay[cell] += ascentchange[cell];
            std::cout << "Katabatic drainage season=" << season << " active_area_fraction=" << activeArea / area
                << " mean_deficit_k=" << deficitArea / std::max(1.0, activeArea) << " max_deficit_k=" << maximumDeficit
                << " mean_speed_change_mps=" << speedChangeArea / std::max(1.0, activeArea)
                << " max_force_mps2=" << maximumForce << " mean_power_wm2=" << powerArea / area
                << " transport_increment_fraction=" << drainageMean
                << " shallow_10m=" << atmosphere::katabaticShallowTenMetreDiagnostic
                << " cold_depth_m=" << (atmosphere::katabaticShallowTenMetreDiagnostic ?
                    atmosphere::katabaticShallowDepthMetres : atmosphere::katabaticVelocityDecayHeightMetres)
                << " ascent_integral=" << climategrid::areaWeightedIntegral(columns, rows, ascentchange)
                << " seconds=" << std::chrono::duration<double>(std::chrono::steady_clock::now() - started).count() << '\n';
        }
        if (!config.stationaryCaptureDirectory.empty())
        {
            vector<float> thermal(surfaceforcing.size()), diabatic(surfaceforcing.size()), zonalfield(surfaceforcing.size());
            auto radiativePressure = lowerheating;
            vector<float> latentPressure(surfaceforcing.size(), 0.0f), sensiblePressure(surfaceforcing.size(), 0.0f);
            if (climate.hasProcesses && climate.processes[season].durationSeconds > 0.0)
            {
                const auto& fields = climate.processes[season];
                radiativePressure = climategrid::remapField(fields.columns, fields.rows, fields.radiativeHeatingWm2[0], columns, rows);
                latentPressure = climategrid::remapField(fields.columns, fields.rows, fields.latentHeatingWm2[0], columns, rows);
                sensiblePressure = climategrid::remapField(fields.columns, fields.rows, fields.sensibleHeatingWm2, columns, rows);
            }
            vector<float> linearizationSpeed(surfaceforcing.size());
            vector<float> upperZonalHeight(surfaceforcing.size()), upperHeatingHeight(surfaceforcing.size()),
                upperSurfaceHeight(surfaceforcing.size());
            for (int cell = 0; cell < columns * rows; ++cell)
            {
                linearizationSpeed[cell] = config.surfaceBoundaryLayerDepthMetres /
                    (config.surfaceDragCoefficients[cell] * config.surfaceDragTimesSeconds[cell]);
                thermal[cell] = surfaceforcing[cell] - mechanicalforcing[cell] - regionalPressure[cell] - continentalPressure[cell];
                diabatic[cell] = lowerheating[cell] * config.surfaceHeatingPressureResponseHpaPerWm2;
                radiativePressure[cell] *= config.surfaceHeatingPressureResponseHpaPerWm2;
                latentPressure[cell] *= circulation::stationaryLatentHeatProjectionFraction * config.surfaceHeatingPressureResponseHpaPerWm2;
                sensiblePressure[cell] *= config.surfaceHeatingPressureResponseHpaPerWm2;
                zonalfield[cell] = zonalpressure[cell / columns];
                upperZonalHeight[cell] = config.zonalUpperHeightMetres[cell / columns];
                upperHeatingHeight[cell] = upperheating[cell] * config.upperHeatingHeightResponseMetresPerWm2;
                upperSurfaceHeight[cell] = (diabatic[cell] + surfaceforcing[cell]) *
                    config.surfaceToUpperCoupling / pressurepermetre;
            }
            climateatmosphere::detail::captureCirculation(config.stationaryCaptureDirectory, columns, rows, season,
                {{"temperature_c", temperature}, {"land_fraction", land}, {"terrain_m", terrain},
                 {"pressure_linearization_speed_mps", linearizationSpeed},
                 {"permanent_ice_fraction", permanentice}, {"drag_coefficient", config.surfaceDragCoefficients},
                 {"persistent_snow_fraction", persistentsnow},
                 {"zonal_hpa", zonalfield}, {"mechanical_hpa", mechanicalforcing}, {"thermal_hpa", thermal},
                 {"continental_eddy_hpa", continentalPressure},
                 {"regional_trough_hpa", regionalPressure}, {"regional_trough_latitude", regionalTrough},
                 {"diabatic_hpa", diabatic}, {"filtered_equilibrium_hpa", result.surfaceStationarySolver.equilibriumPressureAnomalyHpa},
                 {"diabatic_radiative_hpa", radiativePressure}, {"diabatic_latent_hpa", latentPressure},
                 {"diabatic_sensible_hpa", sensiblePressure},
                 {"upper_zonal_height_m", upperZonalHeight}, {"upper_heating_height_m", upperHeatingHeight},
                 {"upper_orographic_height_m", config.upperOrographicHeightMetres},
                 {"upper_wave_terrain_m", upperWaveTerrain},
                 {"upper_surface_coupling_height_m", upperSurfaceHeight},
                 {"upper_filtered_equilibrium_hpa", result.upperStationarySolver.equilibriumPressureAnomalyHpa},
                 {"upper_solved_height_m", result.upperHeightAnomalyMetres},
                 {"upper_east_mps", result.upperEastWindMps}, {"upper_south_mps", result.upperSouthWindMps},
                 {"solved_hpa", result.surfacePressureAnomalyHpa}, {"east_mps", result.surfaceEastWindMps},
                 {"native_solved_hpa", nativeSolvedPressure},
                 {"south_mps", result.surfaceSouthWindMps}, {"thermal_input_c", thermaltemperature},
                 {"skin_c", diagnosticSkin}, {"snow_ice_fraction", diagnosticSnow}, {"drainage_deficit_k", diagnosticDeficit},
                 {"drainage_east_mps2", diagnosticForceEast}, {"drainage_north_mps2", diagnosticForceNorth},
                 {"pre_drainage_east_mps", preDrainageEast}, {"pre_drainage_south_mps", preDrainageSouth},
                 {"pre_advection_east_mps", preAdvectionEast}, {"pre_advection_south_mps", preAdvectionSouth},
                 {"low_level_850_east_mps", wind850East}, {"low_level_850_south_mps", wind850South},
                 {"low_level_850_pressure_hpa", pressure850}, {"low_level_850_available", available850},
                 {"low_level_850_thermal_hpa", thermal850},
                 {"pre_850_surface_east_mps", pre850SurfaceEast}, {"pre_850_surface_south_mps", pre850SurfaceSouth},
                 {"pre_exchange_850_east_mps", preExchange850East}, {"pre_exchange_850_south_mps", preExchange850South},
                 {"native_850_east_mps", native850East}, {"native_850_south_mps", native850South},
                 {"process_fields_available", vector<float>(columns * rows, climate.hasProcesses ? 1.0f : 0.0f)},
                 {"radiative_deficit_k", diagnosticRadiativeDeficit}, {"surface_radiative_wm2", diagnosticRadiation},
                 {"drainage_transport_fraction", vector<float>(columns * rows, drainageMean)},
                 {"cold_layer_depth_m", vector<float>(columns * rows, atmosphere::katabaticShallowTenMetreDiagnostic ?
                     atmosphere::katabaticShallowDepthMetres : atmosphere::katabaticVelocityDecayHeightMetres)},
                 {"shallow_10m_enabled", vector<float>(columns * rows, atmosphere::katabaticShallowTenMetreDiagnostic ? 1.0f : 0.0f)}});
        }
        ...
        floatgrid carrieru, carrierv;
        if (!carrierEast.empty()) { carrieru = output(carrierEast); carrierv = output(carrierSouth); }
        if (!climate.transientWeather && !config.boundaryLayer.enabled && config.enabled.surface &&
            climategrid::validGlobalGridDimensions(width + 1, height + 1))
        {
            // Interpolate the native effective acceleration, then apply local
            // drag. Averaging wind and Cd separately changes quadratic stress.
            vector<float> nativeeast(columns * rows), nativenorth(columns * rows);
            vector<float> carrierForceEast(carrierEast.size()), carrierForceNorth(carrierEast.size());
            for (int y = 0; y < rows; ++y)
                for (int x = 0; x < columns; ++x)
                {
                    const int cell = y * columns + x;
                    const auto force = climateatmosphere::detail::surfaceMomentumForce(
                        {result.surfaceEastWindMps[cell], result.surfaceSouthWindMps[cell]},
                        config.surfaceDragCoefficients[cell], latitudeforrow(y, rows - 1), config);
                    nativeeast[cell] = force.eastMps2;
                    nativenorth[cell] = force.northMps2;
                    if (!carrierEast.empty())
                    {
                        const auto carrierForce = climateatmosphere::detail::surfaceMomentumForce(
                            {carrierEast[cell], carrierSouth[cell]}, config.surfaceDragCoefficients[cell],
                            latitudeforrow(y, rows - 1), config);
                        carrierForceEast[cell] = carrierForce.eastMps2;
                        carrierForceNorth[cell] = carrierForce.northMps2;
                    }
                }
            const auto eastforce = output(nativeeast), northforce = output(nativenorth);
            const auto carrierEastForce = carrierEast.empty() ? floatgrid{} : output(carrierForceEast);
            const auto carrierNorthForce = carrierEast.empty() ? floatgrid{} : output(carrierForceNorth);
            // Reconstruct background and buoyancy separately: reinterpreting a
            // shallow 10 m wind as a 300 m force would undo its momentum closure.
            const auto coldEastForce = atmosphere::katabaticShallowTenMetreDiagnostic ? output(diagnosticForceEast) : floatgrid{};
            const auto coldNorthForce = atmosphere::katabaticShallowTenMetreDiagnostic ? output(diagnosticForceNorth) : floatgrid{};
            const auto outputgrid = climategrid::makeSphericalGrid(
                width + 1, height + 1, config.planetRadiusMetres);
            vector<float> deltau((width + 1) * (height + 1)), deltav(deltau.size());
            for (int y = 0; y <= height; ++y)
                for (int x = 0; x <= width; ++x)
                {
                    const float drag = surfacedrag(world.nom(x, y) > world.sealevel() ? 1.0f : 0.0f,
                        static_cast<float>(std::max(0, world.nom(x, y) - world.sealevel())),
                        std::max(worldpermanentlandicefraction(world,x,y), atmosphere::persistentSnowDragFraction *
                            persistentsnowoutput[y * (width + 1) + x]), vegetationcontrast[y * (width + 1) + x]);
                    auto corrected = climateatmosphere::steadyQuadraticDragCoriolisWind(
                        eastforce[x][y], northforce[x][y], latitudeforrow(y, height), drag,
                        config.surfaceBoundaryLayerDepthMetres,
                        config.rotationRatePerSecond, config.rotationDirection);
                    const int cell = y * (width + 1) + x;
                    deltau[cell] = corrected.eastMetresPerSecond - windu[x][y];
                    deltav[cell] = corrected.southMetresPerSecond - windv[x][y];
                    if (!carrierEast.empty())
                    {
                        const auto carrier = climateatmosphere::steadyQuadraticDragCoriolisWind(
                            carrierEastForce[x][y], carrierNorthForce[x][y], latitudeforrow(y, height), drag,
                            config.surfaceBoundaryLayerDepthMetres, config.rotationRatePerSecond, config.rotationDirection);
                        if (atmosphere::katabaticShallowTenMetreDiagnostic)
                        {
                            corrected = climateatmosphere::detail::coldJetTenMetreWind(carrier,
                                {coldEastForce[x][y], coldNorthForce[x][y]}, latitudeforrow(y, height),
                                drag * atmosphere::katabaticTenMetreStressFraction, atmosphere::katabaticShallowDepthMetres,
                                config.rotationRatePerSecond, config.rotationDirection);
                            deltau[cell] = corrected.eastMetresPerSecond - windu[x][y];
                            deltav[cell] = corrected.southMetresPerSecond - windv[x][y];
                        }
                        // Correct convergence of the layer mean, while retaining
                        // the full local velocity increment in the surface map.
                        deltau[cell] -= (1.0f - drainageMean) * ((corrected.eastMetresPerSecond -
                            carrier.eastMetresPerSecond) - (windu[x][y] - carrieru[x][y]));
                        deltav[cell] -= (1.0f - drainageMean) * ((corrected.southMetresPerSecond -
                            carrier.southMetresPerSecond) - (windv[x][y] - carrierv[x][y]));
                        carrieru[x][y] = carrier.eastMetresPerSecond;
                        carrierv[x][y] = carrier.southMetresPerSecond;
                    }
                    windu[x][y] = corrected.eastMetresPerSecond;
                    windv[x][y] = corrected.southMetresPerSecond;
                }
            // Retain the legacy upper contribution and update lower convergence
            // with the same finite-volume mass-flux diagnosis as the atmosphere.
            const auto ascentchange = climateatmosphere::detail::surfaceAscentCorrectionHpaPerDay(
                outputgrid, deltau, deltav, 0.5f * config.surfaceEquivalentPressureDepthHpa);
            for (int y = 0; y <= height; ++y)
                for (int x = 0; x <= width; ++x)
                    vertical[x][y] += ascentchange[y * (width + 1) + x];
        }
        // No raster-scale pressure relaxation may erase the accepted modes.
        if (!carrierEast.empty())
        {
            auto bulku = windu, bulkv = windv;
            auto du = windu, dv = windv;
            for (int y = 0; y <= height; ++y)
                for (int x = 0; x <= width; ++x)
                {
                    du[x][y] -= carrieru[x][y]; dv[x][y] -= carrierv[x][y];
                    bulku[x][y] -= (1.0f - drainageMean) * du[x][y];
                    bulkv[x][y] -= (1.0f - drainageMean) * dv[x][y];
                }
            climate.winds.drainagewindu[season] = std::move(du);
            climate.winds.drainagewindv[season] = std::move(dv);
            climate.winds.hasDrainageProfile[season] = true;
            storeterrainverticalmotion(world, season, orographicterrain, bulku, bulkv);
        }
        else storeterrainverticalmotion(world, season, orographicterrain, windu, windv);
        climate.winds.surfacewindu[season] = windu;
        climate.winds.surfacewindv[season] = windv;
        climate.winds.upperwindu[season] = upperu;
        climate.winds.upperwindv[season] = upperv;
        climate.winds.lowlevelwindu[season] = output(wind850East);
        climate.winds.lowlevelwindv[season] = output(wind850South);
        climate.winds.lowlevelavailable[season] = output(available850);
        climate.winds.hasLowLevelProfile[season] = true;
        climate.winds.populated[season] = true;
        ...
        std::cout << "Mode circulation season " << season << ": grid=" << columns << 'x' << rows
            ...
            << " hadley_deg=" << hadleywidth << " thermal_equator_deg=" << confinedtrough
            << " control_thermal_equator_deg=" << thermalequator
            << " adjusted_zonal_pressure=" << config.adjustZonalSurfacePressure
            << " adjusted_zonal_upper=" << config.adjustZonalUpperHeight
        ...
        {
            const auto& solver = mode ? result.upperStationarySolver : result.surfaceStationarySolver;
            std::cout << "Atmosphere solver season=" << season << " mode=" << (mode ? "upper" : "surface")
                << " columns=" << columns << " rows=" << rows
                << " method=" << static_cast<int>(config.stationarySolver)
                << " spatial=" << solver.usedSpatialPreconditioner << " iterations=" << solver.iterations
                << " iteration_limit=" << config.maximumIterations << " tolerance=" << config.relativeTolerance
                << " residual=" << solver.relativeResidual << " returned_residual=" << solver.returnedRelativeResidual
                << " operator_seconds=" << solver.operatorSeconds << " preconditioner_seconds=" << solver.preconditionerSeconds
                << " orthogonalization_seconds=" << solver.orthogonalizationSeconds << '\n';
        }
    }
...
void createadvectedrainfall(planet& world, vector<vector<int>>& inland, vector<vector<int>>& fractal)
{
    ...
    (void)inland;
    (void)fractal;
    const bool diurnalCycle = climate.transientWeather &&
        tuning::climate::moistureadvection::enablePrescribedCoastalDiurnalCycle;
    const bool prescribedSynoptic = climate.transientWeather &&
    ...
    const climatehydrology::ClimateGridDimensions climatedimensions =
        climatehydrology::climateGridDimensions(
            outputwidth + 1,
            outputheight + 1,
            climate.resolution.hydrologyColumns);
    const int width = climatedimensions.columns - 1;
    const int height = climatedimensions.rows - 1;
    if (tuning::climate::moistureadvection::oceanEvaporationUseBoundaryHumidity)
        std::cout << "Ocean evaporation humidity=lower-reservoir proxy\n";
    const auto* referenceqa = referenceexperiment().field(width + 1, "qa");
    const auto* referenceevapmonthly = referenceexperiment().field(width + 1, "evap_m");
    const auto* referenceevap = referenceevapmonthly ? referenceevapmonthly : referenceexperiment().field(width + 1, "evap");
    const auto* referenceradiation = referenceexperiment().field(width + 1, "rad");
    const auto* referenceuppermodeeast = referenceexperiment().field(width + 1, "ut500");
    const auto* referenceuppermodenorth = referenceexperiment().field(width + 1, "vt500");
    if (bool(referenceuppermodeeast) != bool(referenceuppermodenorth))
        throw std::runtime_error("Reference upper transport mode requires both wind components");
    // Diagnostic ledgers are separate from prognostic stores and saved-world fields.
    std::array<std::vector<std::array<double, 5>>, CLIMATESEASONCOUNT> referenceledger;
    if (referenceqa || referenceevap || referenceradiation)
        for (auto& quarter : referenceledger) quarter.resize((width + 1) * (height + 1));
    std::array<std::vector<std::array<double, 3>>, CLIMATESEASONCOUNT> uppertransportledger;
    if (referenceuppermodeeast)
        for (auto& quarter : uppertransportledger) quarter.resize((width + 1) * (height + 1));
    const int sealevel = world.sealevel();
    const float gravitymultiplier = std::max(0.05f, world.gravity());
    std::array<std::vector<float>, 12> monthlyrain;
    const bool hasfloatingwinds = climate.winds.completeFor(world.width(), world.height());
    ...
    for (int y = 0; y <= height; y++)
    {
        ...
        rowareaweights[y] = climatehydrology::climateCellAreaWeight(
            y,
            height + 1);
        // Cell-face polar boundaries close transport; do not suppress valid
        // meridional transport over whole high-latitude bands.
    ...
    const auto inputremap = climategrid::makeConservativeRemap(
        outputwidth + 1, outputheight + 1, width + 1, height + 1);
    ...
    hydrologyfloatgrid landpermanentice(width + 1, height + 1, 0.0f);
    ...
    parallelforrows(0, height, [&](int startrow, int endrow)
    {
        for (int y = startrow; y <= endrow; y++)
        {
            for (int x = 0; x <= width; x++)
            {
                ...
                landpermanentice[x][y] = aggregateworldfield(x,y,
                    [&](int xx, int yy) { return worldpermanentlandicefraction(world,xx,yy); });
            }
        }
    });
    ...
    std::cout
        ...
        << " weather_phases="
        << (diurnalCycle ? tuning::climate::moistureadvection::weatherPhaseCount : 1)
        << " transport=" << (climate.transportMethod == climatecontext::TransportMethod::Ffsl ? "ffsl" : "mpdata")
        << " forcing=" << (climate.transientWeather ? "transient" : "daily-mean")
        << " circulation_background=" << (climate.transientWeather ? "updated" : "radiative-seasonal")
        << " remap=continuous-conservative"
        << " years_per_update=" << tuning::climate::moistureadvection::yearsPerCouplingUpdate
        << std::endl;
    ...
    climatecontext::RowMajorGrid<double> landSurface(width + 1, height + 1, 0.0);
    climatecontext::RowMajorGrid<double> landDeep(width + 1, height + 1, 0.0);
    climatecontext::RowMajorGrid<double> landInputEnergy(width + 1, height + 1, 0.0);
    for (int y = 0; y <= height; ++y)
        for (int x = 0; x <= width; ++x)
        {
            const double initial = aggregateworldfield(x, y, [&](int xx, int yy) {
                double total = 0.0;
                for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
                    total += world.seasonaltemp(season, xx, yy) / 4.0;
                return total;
            });
            landSurface[x][y] = landDeep[x][y] = initial;
        }
    ...
    if (climate.storage.columns == width + 1 && climate.storage.rows == height + 1)
    {
        boundarymoisture = climate.storage.boundary;
        freemoisture = climate.storage.free;
        soilmoisture = climate.storage.soil;
        snowwater = climate.storage.snow;
        landSurface = climate.storage.landSurfaceC;
        landDeep = climate.storage.landDeepC;
    }
    else
        climate.hydrologyYears = 0;
    ...
    hydrologyfloatgrid surfaceexchangeincrementu(width + 1, height + 1, 0.0f);
    hydrologyfloatgrid surfaceexchangeincrementv(width + 1, height + 1, 0.0f);
    hydrologyfloatgrid phaseexchangewindu(width + 1, height + 1, 0.0f);
    hydrologyfloatgrid phaseexchangewindv(width + 1, height + 1, 0.0f);
    const float drainageTransportMean = tuning::climate::atmosphere::katabaticShallowTenMetreDiagnostic ?
        climateatmosphere::detail::coldJetTransportFraction(
            tuning::climate::atmosphere::katabaticShallowDepthMetres,
            tuning::climate::atmosphere::surfaceBoundaryLayerMomentumDepthMetres) :
        climateatmosphere::detail::katabaticLayerMeanFraction(
        tuning::climate::atmosphere::katabaticVelocityDecayHeightMetres,
        tuning::climate::atmosphere::surfaceBoundaryLayerMomentumDepthMetres);
    ...
    double transportSeconds = 0.0, weatherSeconds = 0.0;
    std::size_t transportCalls = 0, transportCellSubsteps = 0;
    int maximumTransportSubsteps = 0;
    auto runsolver = [&](int month)
    {
        const climatehydrology::CalendarMonth calendar =
            climatehydrology::calendarMonth(month);
        monthlyrain[month].assign((width + 1) * (height + 1), 0.0f);
        ...
        const int stepsperday = std::max({
            static_cast<int>(std::ceil(1.0f / tuning::climate::moistureadvection::maximumClimateStepDays)),
            climate.transientWeather
                ? static_cast<int>(std::ceil(static_cast<float>(referenceiterations) / baselineiterations)) : 1,
            diurnalCycle
                ? tuning::climate::moistureadvection::weatherPhaseCount : 1});
        const int iterations = calendar.days * stepsperday;
            ...
            for (int y = startrow; y <= endrow; y++)
            {
                for (int x = 0; x <= width; x++)
                {
                    ...
                    const float u = hasfloatingwinds
                        ? interpolatecachedwind(climate.winds.surfacewindu)
                        : interpolatefield(
                    ...
                    const float v = hasfloatingwinds
                        ? interpolatecachedwind(climate.winds.surfacewindv)
                        : interpolatefield(
                            [&](int season, int xx, int yy)
                            {
                                return world.seasonalvwind(season, xx, yy);
                            });
                    float upperu = hasfloatingwinds
                        ? interpolatecachedwind(climate.winds.upperwindu)
                        : interpolatefield(
                            [&](int season, int xx, int yy)
                            {
                                return world.seasonalupperuwind(season, xx, yy);
                            });
                    float upperv = hasfloatingwinds
                        ? interpolatecachedwind(climate.winds.upperwindv)
                        : interpolatefield(
                            [&](int season, int xx, int yy)
                            {
                    ...
                    if (sea && climate.hasOcean && !tuning::climate::oceancurrents::oneWayDiagnosticsOnly)
                        icefraction[x][y] = interpolatefield([&](int season, int xx, int yy)
                            { return climate.oceanIce[season][xx][yy]; });
                    ...
                    const bool hasLowLevel = hasfloatingwinds &&
                        climate.winds.hasLowLevelProfile[calendar.firstSeason] &&
                        climate.winds.hasLowLevelProfile[calendar.secondSeason];
                    const float lowAvailability = hasLowLevel
                        ? interpolatecachedwind(climate.winds.lowlevelavailable) : 0.0f;
                    const float lowWeight = climateatmosphere::detail::lowLevelMoistureWeight(
                        tuning::climate::moistureadvection::freeTroposphereLowLevelWindFraction,
                        lowAvailability, elevation, localtemperature + 273.15f, 9.80665f * gravitymultiplier);
                    if (referenceuppermodeeast)
                    {
                        // Replace only the 500 hPa component of the existing moisture
                        // carrier blend. Momentum diagnosis and surface exchange retain
                        // their own winds; the lower/upper moisture partition is unchanged.
                        const float height500 = 287.05f * (localtemperature + 273.15f) /
                            (9.80665f * gravitymultiplier) * std::log(100000.0f / 50000.0f);
                        const float missing = std::numeric_limits<float>::quiet_NaN();
                        const float east = climateio::experiment::sample(referenceuppermodeeast,
                            calendar.firstSeason, calendar.secondSeason, calendar.interpolation, x, y, missing);
                        const float north = climateio::experiment::sample(referenceuppermodenorth,
                            calendar.firstSeason, calendar.secondSeason, calendar.interpolation, x, y, missing);
                        if (elevation < height500 && std::isfinite(east) && std::isfinite(north))
                        {
                            auto& ledger = uppertransportledger[quarter][y * (width + 1) + x];
                            const double seconds = calendar.days * 86400.0;
                            ledger[0] += seconds;
                            ledger[1] += seconds * upperwinddeparturemultiplier * (1.0f - lowWeight) * (east - upperu);
                            ledger[2] += seconds * upperwinddeparturemultiplier * (1.0f - lowWeight) *
                                (north + upperv) * polartransportfactors[y];
                            upperu = east;
                            upperv = -north;
                        }
                    }
                    ...
                    const auto drainage = [&](const auto& field)
                    {
                        if (!hasfloatingwinds || !tuning::climate::atmosphere::separateKatabaticTransport) return 0.0f;
                        return interpolatefield([&](int s, int xx, int yy)
                        { return climate.winds.hasDrainageProfile[s] ? field[s][xx][yy] : 0.0f; });
                    };
                    const float du = drainage(climate.winds.drainagewindu);
                    const float dv = drainage(climate.winds.drainagewindv);
                    if (lowWeight > 0.0f && lowAvailability > 0.0f)
                    {
                        // Normalize masked remapping near level/terrain intersections.
                        // The availability weight then makes the transition continuous.
                        upperu = climateatmosphere::detail::moistureCarrierMode(upperu,
                            interpolatecachedwind(climate.winds.lowlevelwindu) / lowAvailability, lowWeight);
                        upperv = climateatmosphere::detail::moistureCarrierMode(upperv,
                            interpolatecachedwind(climate.winds.lowlevelwindv) / lowAvailability, lowWeight);
                    }
                    surfacewindu[x][y] = (u - (1.0f - drainageTransportMean) * du) * boundarytransportwindfraction;
                    surfacewindv[x][y] = (v - (1.0f - drainageTransportMean) * dv) * boundarytransportwindfraction;
                    // The profile vanishes above the boundary layer. Preserve
                    // the carrier/upper blend, including the existing polar taper.
                    upperwindu[x][y] = (u - du) +
                        (upperu - (u - du)) * upperwinddeparturemultiplier;
                    upperwindv[x][y] = (v - dv) +
                        (upperv - (v - dv)) * upperwinddeparturemultiplier;
                    surfaceexchangeincrementu[x][y] = (1.0f - drainageTransportMean) * du * boundarytransportwindfraction;
                    surfaceexchangeincrementv[x][y] = (1.0f - drainageTransportMean) * dv * boundarytransportwindfraction;
            ...
            float transporttimestepseconds)
        {
            ...
            options.endZonalWindMps = windu.values();
            options.endMeridionalWindMps = windv.values();
            ...
            climatehydrology::SphericalTracerTransportDiagnostics flux;
            if (climate.transportMethod == climatecontext::TransportMethod::Ffsl)
            {
                climatehydrology::FfslOptions ffsl;
                ffsl.endZonalWindMps = windu.values(); ffsl.endMeridionalWindMps = windv.values();
                flux = climatehydrology::advectSphericalTracerFfsl(columns, rows, source.values(),
                    startu.values(), startv.values(), transporttimestepseconds,
                    tuning::climate::moistureadvection::referencePlanetRadiusMetres, ffsl, transported);
            }
            else flux = climatehydrology::advectSphericalTracerMpdata(
                columns,
                rows,
                source.values(),
                startu.values(),
                ...
                options,
                transported);
            transportSeconds += std::chrono::duration<double>(std::chrono::steady_clock::now() - transportStart).count();
            ++transportCalls;
            transportCellSubsteps += flux.cellSubsteps;
            maximumTransportSubsteps = std::max(maximumTransportSubsteps, flux.substeps);
            ...
            destination.values() = std::move(transported);
        };

        const int weathercolumns = std::min(
            width + 1,
            climate.resolution.weatherColumns);
        ...
        const auto weatherbackground = [&](const hydrologyfloatgrid& field)
        {
            ...
            return climategrid::remapField(
                width + 1, height + 1, flat, weathercolumns, weatherrows);
        };
        ...
        bool evolvingweather =
            climate.transientWeather && tuning::climate::moistureadvection::enableEvolvingWeatherAnomalies;
        ...
        const auto firstStepStart = std::chrono::steady_clock::now();
        for (int iteration = 0; iteration < iterations; iteration++)
        {
            ...
            const auto solar = climateenergy::solarForcing(dayofyear, world.tilt(), world.eccentricity(), world.perihelion());
            const float solardeclination = solar.declinationRadians, solardistance = solar.distanceFactor;
            if (evolvingweather)
            {
                const auto weatherStart = std::chrono::steady_clock::now();
                ...
                weatherSeconds += std::chrono::duration<double>(std::chrono::steady_clock::now() - weatherStart).count();
            }
            if (month == 0 && iteration == 0 && isworldgendebugrunactive())
                std::cout << "Climate first-step weather seconds=" << std::chrono::duration<double>(
                    std::chrono::steady_clock::now() - firstStepStart).count() << std::endl;
            const auto buildphase = [&](const climateweather::ShallowWaterState& phaseweather, double phaseseconds)
            {
            parallelforrows(0, height, [&](int startrow, int endrow)
            {
                for (int y = startrow; y <= endrow; y++)
                {
                    ...
                    for (int x = 0; x <= width; x++)
                    {
                        const climatehydrology::WeatherPhase weatherphase =
                            climatehydrology::weatherPhaseAtTime(
                                phaseseconds, -180.0f + 360.0f * (x + 0.5f) / (width + 1),
                                prescribedSynoptic
                        ...
                        const float synopticu = evolvingweather
                            ? std::clamp(
                                ...
                                tuning::climate::moistureadvection::
                                    maximumWeatherAnomalyWindMps)
                            : (prescribedSynoptic ? 1.0f : 0.0f) * tuning::climate::moistureadvection::synopticWindPerturbationMps *
                                std::sin(
                        ...
                        const float coastalwind = tuning::climate::moistureadvection::enablePrescribedCoastalDiurnalCycle
                            ? coastalstrength[x][y] * (diurnalCycle ? weatherphase.coastalDirection * coastalmagnitude
                                : climatehydrology::dailyMeanCycle(
                                    tuning::climate::moistureadvection::daytimeOnshoreCoastalWindMps,
                                    -tuning::climate::moistureadvection::nighttimeOffshoreCoastalWindMps)) : 0.0f;
                        ...
                        const float diurnalanomaly = tuning::climate::moistureadvection::enablePrescribedCoastalDiurnalCycle
                            ? (diurnalCycle ? weatherphase.seaTemperatureAnomalyC * (1.0f - landfraction[x][y]) +
                                weatherphase.landTemperatureAnomalyC * landfraction[x][y]
                                : climatehydrology::dailyMeanCycle(tuning::climate::moistureadvection::daytimeSeaTemperatureAnomalyC,
                                    tuning::climate::moistureadvection::nighttimeSeaTemperatureAnomalyC) * (1.0f - landfraction[x][y]) +
                                    climatehydrology::dailyMeanCycle(tuning::climate::moistureadvection::daytimeLandTemperatureAnomalyC,
                                        tuning::climate::moistureadvection::nighttimeLandTemperatureAnomalyC) * landfraction[x][y]) : 0.0f;
                        // Atmospheric latent/cloud adjustments do not directly
                        // change the independently integrated surface or ocean ice skin.
                        phasesurfacetemperature[x][y] = seacell[x][y]
                            ? temperature[x][y] + diurnalanomaly
                            : static_cast<float>(landSurface[x][y]);
                        ...
                        phaseboundarycapacity[x][y] = phasesaturationcapacity[x][y] *
                            tuning::climate::moistureadvection::boundaryLayerCapacityFraction;
                        // Scalar surface exchange retains the full local jet;
                        // its layer-mean transport velocity may be much smaller.
                        phaseexchangewindu[x][y] = phasewindu[x][y] +
                            surfaceexchangeincrementu[x][y] * rotationcos - surfaceexchangeincrementv[x][y] * rotationsin;
                        phaseexchangewindv[x][y] = phasewindv[x][y] +
                            (surfaceexchangeincrementu[x][y] * rotationsin + surfaceexchangeincrementv[x][y] * rotationcos) *
                            polartransportfactors[y];
                        surfacewindspeed[x][y] = std::max(
                            tuning::climate::moistureadvection::minimumSurfaceWind,
                            std::sqrt(
                                phaseexchangewindu[x][y] * phaseexchangewindu[x][y] +
                                phaseexchangewindv[x][y] * phaseexchangewindv[x][y]));
                    }
                }
            });

            };
            buildphase(startweather, startday * 86400.0 + iteration * timestepseconds);
            ...
            buildphase(weatherstate, startday * 86400.0 + (iteration + 1) * timestepseconds);
            climatesample.elapsedSeconds = startday * 86400.0 + (iteration / stepsperday + 0.5) * 86400.0;
            for (int y = 0; y <= height; ++y)
                for (int x = 0; x <= width; ++x)
                {
                    const int cell = y * (width + 1) + x;
                    climatesample.layers[0].eastWindMps[cell] += 0.5f * (previousphasewindu[x][y] + phasewindu[x][y]) / stepsperday;
                    climatesample.layers[0].southWindMps[cell] += 0.5f * (previousphasewindv[x][y] + phasewindv[x][y]) / stepsperday;
                    climatesample.layers[1].eastWindMps[cell] += 0.5f * (previousphaseupperwindu[x][y] + phaseupperwindu[x][y]) / stepsperday;
                    climatesample.layers[1].southWindMps[cell] += 0.5f * (previousphaseupperwindv[x][y] + phaseupperwindv[x][y]) / stepsperday;
                }
            if ((iteration + 1) % stepsperday == 0)
            {
                seasonalweather[quarter].push_back(climatesample);
                seasonalweights[quarter].push_back(86400.0);
                for (auto& layer : climatesample.layers)
                {
                    std::fill(layer.eastWindMps.begin(), layer.eastWindMps.end(), 0.0f);
                    std::fill(layer.southWindMps.begin(), layer.southWindMps.end(), 0.0f);
                }
            }
            ...
            parallelforrows(0, height, [&](int startrow, int endrow)
            {
                for (int y = startrow; y <= endrow; y++)
                {
                    for (int x = 0; x <= width; x++)
                    {
                        ...
                        float originalhumidity = std::clamp(retained / surfacecapacity, 0.0f, 1.0f);
                        if (sea && tuning::climate::moistureadvection::oceanEvaporationUseBoundaryHumidity)
                            originalhumidity = climatephysics::detail::boundarySurfaceRelativeHumidity(
                                boundarymoisture[x][y], surfacecapacity,
                                tuning::climate::moistureadvection::boundaryLayerCapacityFraction);
                        float relativehumidity = originalhumidity;
                        if (sea && referenceqa)
                        {
                            const float qa = climateio::experiment::sample(referenceqa,
                                calendar.firstSeason, calendar.secondSeason, calendar.interpolation,
                                x, y, std::numeric_limits<float>::quiet_NaN());
                            if (std::isfinite(qa))
                            {
                                const float saturation = climatephysics::saturationSpecificHumidity(
                                    surfacetemperature, surfacepressure[x][y]);
                                relativehumidity = std::clamp(qa / std::max(1.0e-8f, saturation), 0.0f, 1.0f);
                                auto& ledger = referenceledger[quarter][y * (width + 1) + x];
                                ledger[0] += timestepseconds;
                                ledger[1] += qa * timestepseconds;
                                ledger[2] += qa >= saturation ? timestepseconds : 0.0;
                            }
                        }
                        const float airtemperature = localtemperature;
                        // Scalar turbulent fluxes need wind variance missing
                        // from the daily baseline mean vector. Leave transport
                        // and stress vectors unchanged; transient runs opt out.
                        const float exchangespeed = sea && !climate.transientWeather
                            ? climatephysics::surfaceExchangeWindSpeedMps(phaseexchangewindu[x][y], phaseexchangewindv[x][y],
                                tuning::climate::moistureadvection::minimumSurfaceWind,
                                tuning::climate::moistureadvection::baselineOceanUnresolvedWindRmsMps)
                            : surfacewindspeed[x][y];
                        const float frozencover = sea ? std::clamp(icefraction[x][y], 0.0f, 1.0f)
                            : std::max(landpermanentice[x][y], std::clamp(snowwater[x][y] /
                        ...
                        sensibleheat[x][y] = tuning::climate::atmosphere::surfaceAirDensityKgM3 * 1004.0f *
                            transfercoefficient * exchangespeed * (surfacetemperature - airtemperature);
                        const float potentialevaporation =
                            ...
                            transfercoefficient);
                        if (sea && referenceqa)
                            referenceledger[quarter][y * (width + 1) + x][3] += potentialevaporation -
                                climatephysics::bulkAerodynamicEvaporationMmAtPressure(surfacetemperature,
                                    surfacepressure[x][y], exchangespeed, originalhumidity, timestepseconds, transfercoefficient);
                        ...
                        if (sea)
                        {
                            ...
                            if (referenceevap)
                            {
                                const float rate = referenceevapmonthly ? referenceevapmonthly->value(month, x, y) :
                                    climateio::experiment::sample(referenceevap,
                                    calendar.firstSeason, calendar.secondSeason, calendar.interpolation,
                                    x, y, std::numeric_limits<float>::quiet_NaN());
                                if (std::isfinite(rate) && rate != -9999.9f)
                                {
                                    // The reference is already area/time-mean evaporation, including ice.
                                    // Native condensation is unsupported; negative imposed rates are recorded.
                                    auto& ledger = referenceledger[quarter][y * (width + 1) + x];
                                    const float imposed = std::max(0.0f, rate) * timestepseconds / 86400.0f;
                                    ledger[0] += referenceqa ? 0.0 : timestepseconds;
                                    ledger[1] += referenceqa ? 0.0 : rate * timestepseconds;
                                    ledger[2] += rate < 0.0f ? timestepseconds : 0.0;
                                    ledger[3] += imposed - seaevaporation;
                                    seaevaporation = imposed;
                                }
                            }
            ...
            if (month == 0 && iteration == 0 && isworldgendebugrunactive())
                std::cout << "Climate first-step through transport seconds=" << std::chrono::duration<double>(
                    std::chrono::steady_clock::now() - firstStepStart).count() << std::endl;
            damphighwavenumbers(
                nextboundarymoisture,
                highpasslaplacian,
                ...
                width,
                height,
                tuning::climate::moistureadvection::vapourHighWavenumberDamping * timestepseconds / 86400.0f,
                ...
                for (int y = startrow; y <= endrow; y++)
                {
                    for (int x = 0; x <= width; x++)
                    {
                        ...
                        const float upwardfraction = std::clamp(
                            backgroundexchange +
                                climatehydrology::detail::dailyFractionForTimeStep(
                        ...
                        const double falloutfraction = condensate > 0.0f ? precipitation / condensate : 0.0;
                        exchanges.surfacePrecipitationMm[0][cell] += partition.stratiformMm * falloutfraction;
                        exchanges.surfacePrecipitationMm[1][cell] += partition.orographicMm * falloutfraction;
                        exchanges.surfacePrecipitationMm[2][cell] += partition.convectiveMm * falloutfraction;
                        exchanges.relativeHumiditySeconds[0][cell] += boundaryrelativehumidity * timestepseconds;
                        exchanges.relativeHumiditySeconds[1][cell] += freerelativehumidity * timestepseconds;
                        exchanges.upliftMetreSeconds[cell] += uplift[x][y] * timestepseconds;
                        exchanges.descentMetreSeconds[cell] += descent[x][y] * timestepseconds;
                        exchanges.terrainCoolingCSeconds[cell] += terraincooling * timestepseconds;
                        exchanges.terrainWarmingCSeconds[cell] += terrainwarming * timestepseconds;
                        ...
                        if (!seacell[x][y] && landpermanentice[x][y] > 0.0f)
                            heatinput.surfaceAlbedo = climateatmosphere::detail::frozenLandAlbedo(
                                frozencover, landpermanentice[x][y], static_cast<float>(openalbedo),
                                static_cast<float>(frozenalbedo), static_cast<float>(tuning::climate::energybalance::permanentIceAlbedo));
                        ...
                        auto heat = climateatmosphere::diagnoseColumnHeating(heatinput);
                        if (seacell[x][y] && referenceradiation)
                        {
                            const float imposed = referenceradiation->value(month, x, y);
                            if (std::isfinite(imposed) && imposed != -9999.9f)
                            {
                                const double external = imposed - heat.surfaceRadiativeWm2;
                                heat.surfaceRadiativeWm2 = imposed;
                                heat.surfaceNetHeatingWm2 += external;
                                referenceledger[quarter][cell][4] += external * timestepseconds;
                                // Native column residual remains the residual after subtracting this
                                // explicitly recorded external surface-energy supply. Atmospheric
                                // radiation is deliberately left free in this surface-only experiment.
                            }
                        }
                        for (int layer = 0; layer < 2; ++layer)
                        {
                        ...
                        exchanges.surfaceTemperatureC[cell] += phasesurfacetemperature[x][y] * timestepseconds;
                        exchanges.landSnowCoverFraction[cell] += (seacell[x][y] ? 0.0f : frozencover) * timestepseconds;
                        exchanges.permanentLandIceFraction[cell] += (seacell[x][y] ? 0.0f : landpermanentice[x][y]) * timestepseconds;
                        exchanges.surfaceNetHeatingWm2[cell] += static_cast<float>(heat.surfaceNetHeatingWm2 * timestepseconds);
                        exchanges.surfaceRadiativeHeatingWm2[cell] += static_cast<float>(heat.surfaceRadiativeWm2 * timestepseconds);
                        exchanges.surfaceLatentHeatLossWm2[cell] += 2.5e6f * surfaceevaporation[x][y];
                        exchanges.columnEnergyResidualWm2[cell] += static_cast<float>(heat.closureResidualWm2 * timestepseconds);
                        if (!seacell[x][y])
                        {
                            const auto thermal = climateenergy::stepLandThermalState(
                                {landSurface[x][y], landDeep[x][y]}, heat.surfaceNetHeatingWm2,
                                tuning::climate::energybalance::landHeatCapacityJm2K,
                                tuning::climate::energybalance::landDeepLayerHeatCapacityJm2K,
                                tuning::climate::energybalance::landDeepLayerCouplingWm2K, timestepseconds);
                            landSurface[x][y] = thermal.surfaceC;
                            landDeep[x][y] = thermal.deepC;
                            landInputEnergy[x][y] += heat.surfaceNetHeatingWm2 * timestepseconds;
                        }
        ...
        parallelforrows(0, height, [&](int startrow, int endrow)
        {
            for (int y = startrow; y <= endrow; y++)
            {
                for (int x = 0; x <= width; x++)
                {
                    ...
                    monthlyrain[month][y * (width + 1) + x] = rawrain;
    ...
    for (int cycle = 0; cycle < tuning::climate::moistureadvection::yearsPerCouplingUpdate; cycle++)
    {
        transportSeconds = weatherSeconds = 0.0;
        transportCalls = transportCellSubsteps = 0;
        maximumTransportSubsteps = 0;
        ...
        for (auto& fields : seasonalprocess)
        {
            ...
            fields.surfaceRadiativeHeatingWm2.assign(cells, 0.0f);
            fields.surfaceLatentHeatLossWm2.assign(cells, 0.0f);
            fields.surfaceTemperatureC.assign(cells, 0.0f);
            fields.landSnowCoverFraction.assign(cells, 0.0f);
            fields.permanentLandIceFraction.assign(cells, 0.0f);
            ...
            for (auto& values : fields.surfacePrecipitationMm) values.assign(cells, 0.0);
            for (auto& values : fields.relativeHumiditySeconds) values.assign(cells, 0.0);
            fields.upliftMetreSeconds.assign(cells, 0.0);
            fields.descentMetreSeconds.assign(cells, 0.0);
            fields.terrainCoolingCSeconds.assign(cells, 0.0);
            fields.terrainWarmingCSeconds.assign(cells, 0.0);
        }
        ...
        const auto initialLandSurface = landSurface;
        const auto initialLandDeep = landDeep;
        landInputEnergy.fill(0.0);
        ...
        double landarea = 0.0, landChange = 0.0, landResidual = 0.0;
        ...
        for (int y = 0; y <= height; y++)
        {
            ...
            for (int x = 0; x <= width; x++)
            {
                ...
                {
                    landarea += areaweight;
                    const double ds = landSurface[x][y] - initialLandSurface[x][y];
                    const double dd = landDeep[x][y] - initialLandDeep[x][y];
                    landChange += areaweight * std::max(ds * ds, dd * dd);
                    landResidual += areaweight * std::abs(
                        tuning::climate::energybalance::landHeatCapacityJm2K * ds +
                        tuning::climate::energybalance::landDeepLayerHeatCapacityJm2K * dd - landInputEnergy[x][y]);
        ...
        diagnostics.cyclesCompleted = ++climate.hydrologyYears;
        ...
        climate.landAnnualDriftK = std::sqrt(landChange / std::max(1.0e-30, landarea));
        climate.landEnergyResidualJm2 = landResidual / std::max(1.0e-30, landarea);
        diagnostics.converged = climate.landAnnualDriftK <= 0.05 && diagnostics.cyclesCompleted >=
        ...
        std::cout
            ...
            << " land_drift_k=" << climate.landAnnualDriftK
            << " land_energy_residual_jm2=" << climate.landEnergyResidualJm2
            << " elapsed_seconds=" << cycleelapsedseconds
            << " transport_seconds=" << transportSeconds
            << " weather_seconds=" << weatherSeconds
            << " transport_calls=" << transportCalls
            << " transport_cell_substeps=" << transportCellSubsteps
            << " transport_max_substeps=" << maximumTransportSubsteps
    ...
    const auto outputremap = climategrid::makeSmoothRemap(
        width + 1, height + 1, outputwidth + 1, outputheight + 1);
    auto& monthly = climate.monthly;
    monthly.columns = outputwidth + 1; monthly.rows = outputheight + 1;
    for (int month = 0; month < 12; ++month)
        monthly.rainfallMm[month] = climategrid::remapField(outputremap, monthlyrain[month], width + 1);
    for (auto& field : monthly.referenceRainfallMm) field.resize((outputwidth + 1) * (outputheight + 1));
        ...
        for (int y = startrow; y <= endrow; y++)
        {
            for (int x = 0; x <= outputwidth; x++)
            {
                outputrawannualrain[x][y] = samplehydrologygrid(
                    rawannualrain, outputremap, x, y);
                for (int month = 0; month < 12; ++month)
                {
                    const float rain = monthly.rainfallMm[month][y * (outputwidth + 1) + x];
                    if (rain > outputrawmaximummonthlyrain[x][y])
                    {
                        outputrawmaximummonthlyrain[x][y] = rain;
                        outputrawmaximumrainmonth[x][y] = static_cast<signed char>(month);
                    }
                }
                ...
                for (int season = 0; season < CLIMATESEASONCOUNT; season++)
                {
                    const float seasonalrain = std::max(
                        0.0f,
                        samplehydrologygrid(
                            storedseasonalrain[season], outputremap, x, y));
                    const float seasonalmoisture = samplehydrologygrid(
                        storedseasonalmoisture[season], outputremap, x, y);
                    const float seasonalconvergence = samplehydrologygrid(
                        storedseasonalconvergence[season], outputremap, x, y);
                    world.setseasonalrainfloat(season, x, y, seasonalrain);
                    monthly.referenceRainfallMm[season][y * (outputwidth + 1) + x] = seasonalrain;
                    world.setseasonalmoisture(
                        season,
                        x,
                        y,
    ...
    climate.storage = { width + 1, height + 1, boundarymoisture, freemoisture, soilmoisture, snowwater, landSurface, landDeep };
    climate.hasProcesses = true;
    if (referenceqa || referenceevap || referenceradiation)
    {
        const auto path = referenceexperiment().directory() /
            ("humidity-use-year-" + std::to_string(climate.hydrologyYears) + ".csv");
        std::ofstream output(path);
        output << "season,x,y,seconds,qa_seconds,suppressed_evaporation_seconds,potential_evaporation_delta_mm,external_surface_radiation_jm2\n";
        // For evaporation-only cases qa_seconds stores rate*time and suppression records negative rates.
        output << std::setprecision(10);
        for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
            for (int y = 0; y <= height; ++y)
                for (int x = 0; x <= width; ++x)
                {
                    const auto& ledger = referenceledger[season][y * (width + 1) + x];
                    output << season << ',' << x << ',' << y;
                    for (double value : ledger) output << ',' << value;
                    output << '\n';
                }
        if (!output) throw std::runtime_error("Failed to write reference humidity ledger");
    }
    if (referenceuppermodeeast)
    {
        const auto path = referenceexperiment().directory() /
            ("upper-transport-use-year-" + std::to_string(climate.hydrologyYears) + ".csv");
        std::ofstream output(path);
        output << "season,x,y,seconds,east_velocity_delta_m,north_velocity_delta_m\n" << std::setprecision(10);
        for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
            for (int y = 0; y <= height; ++y)
                for (int x = 0; x <= width; ++x)
                {
                    output << season << ',' << x << ',' << y;
                    for (double value : uppertransportledger[season][y * (width + 1) + x]) output << ',' << value;
                    output << '\n';
                }
        if (!output) throw std::runtime_error("Failed to write reference upper transport ledger");
    }
    for (int season = 0; season < CLIMATESEASONCOUNT; ++season)
    {
        ...
        normalize(fields.sensibleHeatingWm2); normalize(fields.surfaceNetHeatingWm2); normalize(fields.surfaceTemperatureC);
        normalize(fields.landSnowCoverFraction);
        normalize(fields.permanentLandIceFraction);
        normalize(fields.surfaceRadiativeHeatingWm2); normalize(fields.surfaceLatentHeatLossWm2);
        ...
        climate.processes[season] = std::move(fields);
        for (int layer = 0; layer < 2; ++layer)
