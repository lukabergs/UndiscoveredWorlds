...
ShallowWaterState resampleState(const ShallowWaterState& state, int columns)
{
    ...
    const auto remap = [&](const auto& field)
    {
        return climategrid::remapField(
            state.columns, state.rows, field, result.columns, result.rows);
    };
    ...
    const std::vector<double>& sampleDurationsSeconds)
{
    ...
    std::vector<float> speedAnomaly(states.size());
    for (std::size_t cell = 0; cell < cellCount; cell++)
    {
        ...
        for (std::size_t i = 0; i < states.size(); ++i)
            speedAnomaly[i] = std::hypot(states[i].layers[layer].eastWindMps[cell],
                states[i].layers[layer].southWindMps[cell]) - statistics.meanSpeedMps[cell];
        ...
        if (variance > 1.0e-12)
            for (std::size_t lag = 1; lag < states.size() / 2; ++lag)
            {
                ...
                for (std::size_t i = lag; i < states.size(); ++i)
                {
                    ...
                    covariance += w * speedAnomaly[i] * speedAnomaly[i - lag]; pairWeight += w;
                }
...
{
    if (!climategrid::validGlobalGridDimensions(state.columns, state.rows) ||
        state.layers.empty() || state.layers.size() > 2)
    {
        return {};
    }
    ...
    appendScalar<std::uint32_t>(bytes, 2U);
    ...
    ShallowWaterState& state)
{
    ...
    if (!readScalar(bytes, offset, magic) || !readScalar(bytes, offset, version) ||
        ...
        !readScalar(bytes, offset, layerCount) || magic != 0x57535755U || version != 2U ||
        !climategrid::validGlobalGridDimensions(columns, rows) ||
        columns > 4096 || rows > 2048 || layerCount < 1 || layerCount > 2 ||
