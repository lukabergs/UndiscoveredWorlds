...
struct SeasonalProcessFields
{
    ...
    // Surface radiation warms; latent heat loss is positive for evaporation.
    std::vector<float> surfaceRadiativeHeatingWm2, surfaceLatentHeatLossWm2;
    std::vector<float> surfaceTemperatureC;
    std::vector<float> landSnowCoverFraction; // Seasonal mean, zero over ocean.
    std::vector<float> permanentLandIceFraction; // Diagnosed cover, not an ice-mass reservoir.
    std::vector<float> columnWaterMm, ascentHpaPerDay, columnEnergyResidualWm2;
    // Diagnostic integrals only; these do not feed back into climate state.
    // Precipitation order: stratiform, orographic, convective, after reevaporation.
    std::array<std::vector<double>, 3> surfacePrecipitationMm;
    std::array<std::vector<double>, 2> relativeHumiditySeconds;
    std::vector<double> upliftMetreSeconds, descentMetreSeconds;
    std::vector<double> terrainCoolingCSeconds, terrainWarmingCSeconds;
};
...
// Mean of the signed daytime/nighttime half-cosine amplitudes over one day.
constexpr float dailyMeanCycle(float daytimeAmplitude, float nighttimeAmplitude)
{
    return (daytimeAmplitude + nighttimeAmplitude) * 0.3183098861837907f;
}
...
{
    int substeps = 1;
    // Actual cell updates across the adaptive integration substeps.
    std::size_t cellSubsteps = 0;
...
WeatherPhase weatherPhaseAtTime(double elapsedSeconds, float longitudeDegrees,
    ...
    float nighttimeSeaTemperatureAnomalyC, double daySeconds = 86400.0);
