...
float surfaceExchangeWindSpeedMps(float eastMps, float southMps, float minimumMps, float unresolvedRmsMps)
{
    const float unresolved = std::max(0.0f, unresolvedRmsMps);
    return std::max(minimumMps, std::sqrt(eastMps * eastMps + southMps * southMps + unresolved * unresolved));
}
