...
#include "legacy/thermal_response.hpp"

namespace climateatmosphere
{
enum class StationarySolver { LegacyJacobi, ProjectedJacobi, Zonal };
struct CellSpacing
{
...
struct StationaryWaveResponse
{
    ...
    bool usedSpatialPreconditioner = false;
    float returnedRelativeResidual = 0.0f;
    double operatorSeconds = 0.0, preconditionerSeconds = 0.0, orthogonalizationSeconds = 0.0;
};
...
{
    bool enabled = false;
    float fullLatitudeDegrees = 20.0f;
    float outerLatitudeDegrees = 35.0f;
    float inversionPressurePa = 85000.0f;
    float inversionDensityKgM3 = 1.0f;
    float entrainmentRatePerSecond = 2.0e-5f;
    float oceanFrictionRatePerSecond = 1.5e-5f;
    float inversionDragTimeSeconds = 432000.0f;
};

struct BoundaryLayerState
{
    int columns = 0, rows = 0;
    StationaryWaveResponse inversionSolver;
    std::vector<float> inversionTemperatureC, inversionPressureHpa;
    std::vector<float> inversionEastWindMps, inversionSouthWindMps;
    std::vector<float> thermalPressureHpa, surfacePressureHpa;
};

struct ModeSeparatedCirculationConfig
{
    // Let divergence adjust the zonal surface forcing together with regional
    // pressure, instead of adding an unadjusted trough after the solve.
    bool adjustZonalSurfacePressure = false;
    bool adjustZonalUpperHeight = false;
    BoundaryLayerConfig boundaryLayer;
    std::vector<float> inversionEquilibriumPressureHpa;
    std::vector<float> inversionEastWindMps, inversionSouthWindMps;
    ...
    StationarySolver stationarySolver = StationarySolver::Zonal;
    std::string stationaryCaptureDirectory;
...
struct ModeSeparatedCirculation
{
    ...
    double maximumBoundaryLayerMomentumResidualMps2 = 0.0;
    std::vector<float> ascentHpaPerDay;
};

float boundaryLayerWeight(float latitudeDegrees, const BoundaryLayerConfig& config);
// Linear entrainment/friction plus the quadratic drag retained outside the tropics.
HorizontalWind steadyMixedDragCoriolisWind(float forceEast, float forceNorth,
    float coriolis, float linearDragRate, float quadraticDragPerMetre);
BoundaryLayerState solveBoundaryLayer(int columns, int rows,
    const std::vector<float>& surfaceTemperatureC,
    const std::vector<float>& columnHeatingWm2,
    const std::vector<float>& mechanicalPressureHpa,
...
StationaryWaveResponse solveSteadyStationaryWavePressure(
    ...
    StationarySolver solver = StationarySolver::Zonal,
