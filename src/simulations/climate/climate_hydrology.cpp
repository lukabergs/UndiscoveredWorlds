...
float climateCellLatitudeDegrees(int row, int rowCount)
{
    ...
    return static_cast<float>(climategrid::latitudeCentreRadians(
        row, std::max(1, rowCount)) * radiansToDegrees);
}
...
float climateCellAreaWeight(int row, int rowCount)
{
    ...
    return static_cast<float>(climategrid::latitudeBandMeasure(row, rows) * rows / pi);
}
    ...
    std::vector<float>& destination)
{
    struct FaceFlux
    {
        ...
        double inverseFaceArea = 0.0;
        std::array<std::size_t, 4> transverseCells{}, transverseFaces{};
        bool hasNorthFaces = true;
    };
    ...
    std::vector<std::array<std::size_t, 4>> neighbours(cellCount);
    std::vector<double> inverseCellArea(cellCount);
    for (int y = 0; y < rows; y++)
    {
        ...
        for (int x = 0; x < columns; x++)
        {
            ...
            neighbours[west] = {grid.index(x - 1, y), east,
                grid.index(x, std::max(0, y - 1)), grid.index(x, std::min(rows - 1, y + 1))};
            inverseCellArea[west] = 1.0 / cellArea;
    ...
    for (auto& face : fullStepFaces)
    {
        ...
        const int x = static_cast<int>(face.first % columns);
        const int north = std::max(0, row - 1), south = std::min(rows - 1, row + 1);
        face.inverseFaceArea = 1.0 / (0.5 * (grid.cellAreasSquareMetres[row] +
            grid.cellAreasSquareMetres[face.second / columns]));
        if (face.meridional)
        {
            face.transverseCells = {grid.index(x + 1, row), grid.index(x + 1, south),
                grid.index(x - 1, row), grid.index(x - 1, south)};
            face.transverseFaces = {grid.index(x, row), grid.index(x - 1, row),
                grid.index(x, south), grid.index(x - 1, south)};
        }
        else
        {
            face.transverseCells = {grid.index(x, south), grid.index(x + 1, south),
                grid.index(x, north), grid.index(x + 1, north)};
            face.transverseFaces = {grid.index(x, row), grid.index(x + 1, row),
                grid.index(x, north), grid.index(x + 1, north)};
            face.hasNorthFaces = row > 0;
        }
    ...
    diagnostics.cellSubsteps = cellCount * diagnostics.substeps;
    ...
    const auto localBounds = [&](const std::vector<float>& field)
    {
        for (int y = 0; y < rows; y++)
        {
            for (int x = 0; x < columns; x++)
            {
                ...
                for (const std::size_t neighbour : neighbours[cell])
                {
    ...
    for (int step = 0; step < diagnostics.substeps; step++)
    {
        ...
        for (int pass = 0; pass < diagnostics.correctivePasses; pass++)
        {
            ...
            for (std::size_t cell = 0; cell < cellCount; cell++)
                divergence[cell] *= inverseCellArea[cell];
            for (std::size_t faceIndex = 0; faceIndex < fullStepFaces.size(); faceIndex++)
            {
                ...
                const auto& t = fullFace.transverseCells;
                const auto& f = fullFace.transverseFaces;
                const auto& crossAdvector = fullFace.meridional ? eastAdvector : southAdvector;
                const double transverseAdvector = 0.25 * (crossAdvector[f[0]] + crossAdvector[f[1]] +
                    (fullFace.hasNorthFaces ? crossAdvector[f[2]] + crossAdvector[f[3]] : 0.0));
                const double transverseNumerator = current[t[0]] + current[t[1]] - current[t[2]] - current[t[3]];
                const double transverseDenominator = current[t[0]] + current[t[1]] + current[t[2]] + current[t[3]];
                ...
                const double pseudoAdvector =
                    (std::abs(sweptArea) - sweptArea * sweptArea * fullFace.inverseFaceArea) * gradientRatio -
                    sweptArea * transverseAdvector * fullFace.inverseFaceArea * crossRatio -
...
MeanMoistureTransport meanMoistureTransport(const SeasonalProcessFields& fields, double radiusMetres)
{
    ...
    if (!climategrid::validGlobalGridDimensions(fields.columns, fields.rows) ||
        fields.durationSeconds <= 0.0)
        return result;
...
WeatherPhase weatherPhaseAtTime(double elapsedSeconds, float longitudeDegrees,
    float windRotationDegrees, float daytimeLandTemperatureAnomalyC,
    float nighttimeLandTemperatureAnomalyC, float daytimeSeaTemperatureAnomalyC,
    float nighttimeSeaTemperatureAnomalyC, double daySeconds)
{
    if (!(daySeconds > 0.0) || !std::isfinite(elapsedSeconds) || !std::isfinite(longitudeDegrees)) return {};
    constexpr double pi = 3.14159265358979323846;
    const double phase = 2.0 * pi * std::remainder(elapsedSeconds / daySeconds + longitudeDegrees / 360.0, 1.0);
    const float daylight = static_cast<float>(-std::cos(phase));
    const auto temperature = [&](float day, float night)
    { return daylight >= 0.0f ? daylight * day : -daylight * night; };
    return {static_cast<float>(std::sin(phase) * windRotationDegrees * pi / 180.0),
        static_cast<float>(phase), daylight,
        temperature(daytimeLandTemperatureAnomalyC, nighttimeLandTemperatureAnomalyC),
        temperature(daytimeSeaTemperatureAnomalyC, nighttimeSeaTemperatureAnomalyC)};
}
