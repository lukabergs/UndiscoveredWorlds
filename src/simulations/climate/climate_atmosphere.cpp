...
#include "detail/stationary_preconditioner.hpp"
    ...
    std::vector<float>& field)
{
    ...
    for (int y = 0; y < rows; ++y)
        for (int k = 1; k <= std::min(zonalModes, (columns - 1) / 2); ++k)
        {
            ...
            const double filterWeight = 1.0 - static_cast<double>(k) / (std::min(zonalModes, (columns - 1) / 2) + 1.0);
            a *= filterWeight; b *= filterWeight;
            for (int x = 0; x < columns; ++x)
    ...
    for (int x = 0; x < columns; ++x)
        for (int k = 0; k <= std::min(meridionalModes, rows - 1); ++k)
        {
            ...
            coefficient *= 1.0 - static_cast<double>(k) / (std::min(meridionalModes, rows - 1) + 1.0);
            for (int y = 0; y < rows; ++y)
    ...
    bool removeZonalMean)
{
    ...
    if (!climategrid::validGlobalGridDimensions(longitudeCells, latitudeCells) ||
    ...
    float resolvedForcingScaleMetres)
{
    ...
    if (bruntVaisalaFrequencyPerSecond <= 0.0f || modeDepthMetres <= 0.0f ||
        ...
        !climategrid::validGlobalGridDimensions(longitudeCells, latitudeCells))
    {
        return diagnosis;
    }
    ...
    float fullStrengthLatitudeDegrees)
{
    ...
    for (int y = 0; y < latitudeCells; y++)
    {
        ...
        const float latitudeFactor = fullStrengthLatitudeDegrees <= minimumLatitudeDegrees ? 1.0f : smoothstep(
    ...
    float planetRadiusMetres)
{
    ...
    return {
        ...
        pi * planetRadiusMetres / static_cast<float>(latitudeCells)
    };
}
    ...
    const std::string& captureDirectory)
{
    ...
    if (!captureDirectory.empty())
        detail::StationaryReplay{longitudeCells, latitudeCells, equivalentPressureDepthHpa,
            pressureDampingTimeSeconds, airDensityKgM3, planetRadiusMetres, rotationRatePerSecond,
            rotationDirection, preserveZonalMean, maximumIterations, relativeTolerance, restartLength,
            equilibriumPressureAnomalyHpa, dragTimeSeconds}.write(captureDirectory);
    const auto index = [longitudeCells](int x, int y)
    {
    ...
    const auto applyOperator = [&](const std::vector<double>& pressure, std::vector<double>& result)
    {
        ...
        response.operatorSeconds += std::chrono::duration<double>(std::chrono::steady_clock::now() - started).count();
    };
    ...
    std::unique_ptr<detail::StationaryPreconditioner> spatial;
    if (solver == StationarySolver::Zonal)
    {
        spatial = std::make_unique<detail::StationaryPreconditioner>(longitudeCells, latitudeCells,
            dragTimeSeconds, coupling, airDensityKgM3, planetRadiusMetres,
            rotationRatePerSecond, rotationDirection, static_cast<double>(pi));
        if (!spatial->valid()) spatial.reset();
    }
    response.usedSpatialPreconditioner = static_cast<bool>(spatial);
    response.preconditionerSeconds += std::chrono::duration<double>(std::chrono::steady_clock::now() - setupStarted).count();
    const auto precondition = [&](std::vector<double>& values)
    {
        const auto started = std::chrono::steady_clock::now();
        if (spatial) spatial->solve(values);
        else for (size_t cell = 0; cell < cellCount; ++cell) values[cell] *= inverseDiagonal[cell];
        // Keep every Krylov correction in the same constrained pressure space
        // as the returned field. A post-solve projection alone changes A*p when
        // drag varies in longitude and can invalidate an accepted residual.
        if (solver != StationarySolver::LegacyJacobi) projectPressure(values);
        response.preconditionerSeconds += std::chrono::duration<double>(std::chrono::steady_clock::now() - started).count();
    };
    ...
    while (response.iterations < maximumIterations)
    {
        ...
        for (int column = 0; column < availableColumns; column++)
        {
            ...
            response.orthogonalizationSeconds += std::chrono::duration<double>(std::chrono::steady_clock::now() - orthogonalizationStarted).count();
            ...
            response.iterations++;
            // A small Krylov estimate requests an actual-residual check at the
            // cycle boundary. It never grants physical acceptance by itself.
            if (solver != StationarySolver::LegacyJacobi && std::abs(projectedResidual[column + 1]) <= residualNorm * 1.0e-6)
                break;
        }
    ...
    for (size_t cell = 0; cell < cellCount; ++cell) solution[cell] = response.pressureAnomalyHpa[cell];
    applyOperator(solution, operatorSolution);
    for (size_t cell = 0; cell < cellCount; ++cell) residual[cell] = rightHandSide[cell] - operatorSolution[cell];
    response.returnedRelativeResidual = static_cast<float>(std::sqrt(dot(residual, residual)) / physicalRightHandSideNorm);
    return response;
}
    ...
    const ModeSeparatedCirculationConfig& config)
{
    ...
    if (config.adjustZonalUpperHeight && config.enabled.zonal &&
        config.zonalUpperHeightMetres.size() == static_cast<std::size_t>(latitudeCells))
        for (int y = 0; y < latitudeCells; ++y)
            for (int x = 0; x < longitudeCells; ++x)
                upperEquilibriumPressure[y * longitudeCells + x] +=
                    config.zonalUpperHeightMetres[y] * upperPressurePerMetre;
    if (config.adjustZonalSurfacePressure && config.enabled.zonal)
        for (int y = 0; y < latitudeCells; ++y)
            for (int x = 0; x < longitudeCells; ++x)
                surfaceEquilibrium[y * longitudeCells + x] += zonalSurfacePressureHpa[y];
    if (config.enabled.surface && (config.enabled.stationary ||
        (config.adjustZonalSurfacePressure && config.enabled.zonal)))
    {
        result.surfaceStationarySolver = solveSteadyStationaryWavePressure(
            ...
            !config.adjustZonalSurfacePressure,
            config.maximumIterations,
            config.relativeTolerance,
            config.solverRestartLength, config.stationarySolver, config.stationaryCaptureDirectory);
    }
    else
    {
    ...
    if (config.enabled.upper && (config.enabled.stationary ||
        (config.adjustZonalUpperHeight && config.enabled.zonal)))
    {
        ...
        for (int x = 0; x < longitudeCells; x++)
        {
            ...
            if (config.enabled.surface)
            {
                result.surfacePressureAnomalyHpa[cell] =
                    (config.enabled.zonal && !config.adjustZonalSurfacePressure ? zonalSurfacePressureHpa[y] : 0.0f) +
            ...
            if (config.enabled.upper)
            {
                result.upperHeightAnomalyMetres[cell] =
                    (config.enabled.zonal && !config.adjustZonalUpperHeight && config.zonalUpperHeightMetres.size() ==
    ...
    const ModeSeparatedCirculationConfig& config, ModeSeparatedCirculation& result)
{
    ...
    const double latitudeSpacing = pi * static_cast<double>(config.planetRadiusMetres) /
        static_cast<double>(latitudeCells);
    const bool mixedLayer = config.boundaryLayer.enabled && config.inversionEastWindMps.size() == cellCount &&
        config.inversionSouthWindMps.size() == cellCount;
    result.maximumBoundaryLayerMomentumResidualMps2 = 0.0;
    for (int y = 0; y < latitudeCells; y++)
    {
        ...
        for (int x = 0; x < longitudeCells; x++)
        {
            ...
            if (config.enabled.surface)
            {
                ...
                HorizontalWind wind = steadyQuadraticDragCoriolisWind(
                    ...
                    config.rotationRatePerSecond,
                    config.rotationDirection);
                const float blend = mixedLayer ? boundaryLayerWeight(latitudeDegrees, config.boundaryLayer) : 0.0f;
                if (blend > 0.0f)
                {
                    const float cd = config.surfaceDragCoefficients.size() == cellCount
                        ? config.surfaceDragCoefficients[cell] : config.surfaceDragCoefficient;
                    const float exchange = blend * config.boundaryLayer.entrainmentRatePerSecond;
                    const float linear = exchange + blend * config.boundaryLayer.oceanFrictionRatePerSecond * cd / 0.0013f;
                    const float quadratic = (1.0f - blend) * cd / config.surfaceBoundaryLayerDepthMetres;
                    const float f = coriolisParameterPerSecond(latitudeDegrees, config.rotationRatePerSecond, config.rotationDirection);
                    const float eastForce = -pressureGradientEast / config.airDensityKgM3 + exchange * config.inversionEastWindMps[cell];
                    const float northForce = -pressureGradientNorth / config.airDensityKgM3 - exchange * config.inversionSouthWindMps[cell];
                    wind = steadyMixedDragCoriolisWind(eastForce, northForce, f, linear, quadratic);
                    const double drag = linear + quadratic * std::hypot(wind.eastMetresPerSecond, wind.southMetresPerSecond);
                    // Only the fully active tropical closure: the transition
                    // belt still includes the legacy 500 hPa exchange below.
                    if (blend == 1.0f)
                        result.maximumBoundaryLayerMomentumResidualMps2 = std::max(result.maximumBoundaryLayerMomentumResidualMps2,
                            std::hypot(drag * wind.eastMetresPerSecond + f * wind.southMetresPerSecond - eastForce,
                                -drag * wind.southMetresPerSecond + f * wind.eastMetresPerSecond - northForce));
                }
        ...
        for (int y = 0; y < latitudeCells; ++y)
            for (int component = 0; component < 2; ++component)
            {
                ...
                for (int x = 0; x < longitudeCells; ++x)
                {
                    ...
                    // The tropical surface already exchanges with the diagnosed
                    // inversion; do not also mix it with the 500 hPa mode.
                    const double blend = mixedLayer ? boundaryLayerWeight(
                        90.0f - 180.0f * (y + 0.5f) / latitudeCells, config.boundaryLayer) : 0.0;
                    const double shear = (1.0 - blend) * coupling * (upper[cell] - lower[cell] - rowShear);
    ...
    std::vector<float> pressureFluxEast(cellCount), pressureFluxSouth(cellCount);
    for (int y = 0; y < latitudeCells; ++y)
    {
        const double blend = mixedLayer ? boundaryLayerWeight(
            90.0f - 180.0f * (y + 0.5f) / latitudeCells, config.boundaryLayer) : 0.0;
        const double lowerDepth = (1.0 - blend) * 0.5 * config.surfaceEquivalentPressureDepthHpa +
            blend * (100000.0 - config.boundaryLayer.inversionPressurePa) / 100.0;
        const double upperDepth = (1.0 - blend) * 0.5 * config.upperEquivalentPressureDepthHpa;
        for (int x = 0; x < longitudeCells; ++x)
        {
            const auto i = grid.index(x, y);
            pressureFluxEast[i] = static_cast<float>(lowerDepth * result.surfaceEastWindMps[i] - upperDepth * result.upperEastWindMps[i]);
            pressureFluxSouth[i] = static_cast<float>(lowerDepth * result.surfaceSouthWindMps[i] - upperDepth * result.upperSouthWindMps[i]);
        }
    }
    ...
    for (int y = 0; y < latitudeCells; ++y)
    {
        ...
        for (int x = 0; x < longitudeCells; ++x)
        {
            ...
            const double blend = mixedLayer ? boundaryLayerWeight(
                90.0f - 180.0f * (y + 0.5f) / latitudeCells, config.boundaryLayer) : 0.0;
            const double cd = config.surfaceDragCoefficients.size() == cellCount
                ? config.surfaceDragCoefficients[cell] : config.surfaceDragCoefficient;
            const double surfaceDragRate = (1.0 - blend) * cd / config.surfaceBoundaryLayerDepthMetres * std::sqrt(surfaceSpeed2) +
                blend * config.boundaryLayer.oceanFrictionRatePerSecond * cd / 0.0013;
            result.areaWeightedDragDissipationWm2 += area *
                (surfaceMass * surfaceDragRate * surfaceSpeed2 + upperMass * upperSpeed2 / config.upperDragTimeSeconds);
            ...
            // The tropical layer spans surface to 850 hPa, not the wave mode's
            // equivalent depth. Blend mass fluxes before divergence so the
            // transition and the polar/seam boundaries remain globally closed.
            result.ascentHpaPerDay[cell] = mixedLayer ? static_cast<float>(-86400.0 * divergence(pressureFluxEast, pressureFluxSouth)) :
                static_cast<float>(43200.0 * (config.upperEquivalentPressureDepthHpa * divergence(result.upperEastWindMps, result.upperSouthWindMps) -
