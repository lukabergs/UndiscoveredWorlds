...
#include "detail/subgrid_humidity.hpp"
...
void testSeasonalFluxDiagnostic()
{
    ...
    expect(std::abs(climategrid::areaWeightedIntegral(
        8, 4, diagnostic.convergenceMmPerDay)) < 1.0e-12,
...
void testVariableFlow()
{
    ...
    const double initialMass = climategrid::areaWeightedIntegral(columns, rows, initial);
    auto tracer = initial;
    climatehydrology::MpdataOptions options;
    ...
    for (int sign : {-1, 1})
    {
        ...
        const double finalMass = climategrid::areaWeightedIntegral(columns, rows, next);
...
void testExchangeTimeRefinement();
void testSubgridHumidityConservation();

int main()
{
    const auto phaseAt = [](double time, float longitude = 0.0f)
    { return climatehydrology::weatherPhaseAtTime(time, longitude, 0.0f, 2.0f, -2.0f, 0.35f, -0.35f); };
    expect(near(phaseAt(43200.0).coastalDirection, 1.0f) && near(phaseAt(0.0).coastalDirection, -1.0f),
        "coastal forcing must distinguish local noon from midnight");
    expect(near(phaseAt(0.0, 180.0f).coastalDirection, phaseAt(43200.0).coastalDirection),
        "coastal phase must follow longitude");
    expect(near(phaseAt(31.0 * 86400.0 + 43200.0).coastalDirection, phaseAt(43200.0).coastalDirection),
        "physical diurnal phase must not reset at month boundaries");
    for (int steps : {3, 6, 12})
    {
        double dailyAnomaly = 0.0;
        for (int step = 0; step < steps; ++step)
            dailyAnomaly += phaseAt((step + 0.5) * 86400.0 / steps).landTemperatureAnomalyC / steps;
        expect(std::abs(dailyAnomaly) < 1.0e-6, "subdaily refinement must preserve mean thermal forcing");
    }
    double asymmetricMean = 0.0;
    for (int step = 0; step < 4096; ++step)
        asymmetricMean += climatehydrology::weatherPhaseAtTime((step + 0.5) * 86400.0 / 4096,
            0.0f, 0.0f, 3.0f, -1.0f, 0.0f, 0.0f).landTemperatureAnomalyC / 4096;
    expect(std::abs(asymmetricMean - climatehydrology::dailyMeanCycle(3.0f, -1.0f)) < 1.0e-6,
        "daily-mean baseline must retain the mean of asymmetric coastal cycles");
    testExchangeTimeRefinement();
    testSubgridHumidityConservation();
    ...
    const auto climategrid = climatehydrology::climateGridDimensions(512, 256, 128);
    ...
    const auto nonstandardOutputGrid =
        climatehydrology::climateGridDimensions(600, 300, 64);
...
void testExchangeTimeRefinement()
{
    using climatehydrology::detail::dailyFractionForTimeStep;
    for (const float daily : {0.0f, 0.2f, 0.35f, 0.65f, 0.95f})
    {
        expect(dailyFractionForTimeStep(daily, 86400.0f) == daily,
            "daily exchange must retain the existing coefficient exactly");
        for (const int steps : {2, 4, 8, 24})
        {
            ...
            {
                const auto exchange = climatehydrology::exchangeMoistureLayers(
                    remaining, transferred, dailyFractionForTimeStep(daily, 86400.0f / steps), 0.0f);
                remaining = exchange.boundaryLayerMm;
                transferred = exchange.freeTroposphereMm;
            }
            expect(near(remaining, 20.0f * (1.0f - daily), 2.0e-5f),
                "fixed one-way exchange must not strengthen when the time step is refined");
            expect(near(remaining + transferred, 20.0f, 2.0e-5f),
                "refined vertical exchange must conserve water");
        }
    }
    expect(dailyFractionForTimeStep(0.35f, 0.0f) == 0.0f,
        "zero-duration exchange must leave water unchanged");
}

void testSubgridHumidityConservation()
{
    using climatehydrology::detail::subgridCondensableWater;
    for (const float water : {0.0f, 1.0f, 10.0f, 40.0f})
        for (const float capacity : {0.0f, 1.0f, 10.0f, 40.0f})
            for (const float spread : {0.0f, 0.2f, 0.5f, 1.0f})
            {
                const float excess = subgridCondensableWater(water, capacity, spread);
                expect(excess >= 0.0f && excess <= water,
                    "subcell condensation cannot introduce or remove unavailable water");
                double quadrature = 0.0;
                constexpr int samples = 4096;
                for (int i = 0; i < samples; ++i)
                {
                    const double subcell = water * (1.0 + spread * (2.0 * (i + 0.5) / samples - 1.0));
                    quadrature += std::max(0.0, subcell - capacity) / samples;
                }
                expect(std::abs(excess - quadrature) < 2.0e-5,
                    "analytic condensation must match the same-mean subcell humidity distribution");
                if (spread == 0.0f)
                    expect(excess == std::max(0.0f, water - capacity),
                        "zero humidity variance must recover resolved saturation adjustment");
            }
    expect(subgridCondensableWater(9.0f, 10.0f, 0.2f) > 0.0f,
        "a partly saturated cell can condense while its mean remains below saturation");
    expect(subgridCondensableWater(1.0f, 10.0f, 0.2f) == 0.0f,
        "dry cells must not acquire an artificial precipitation floor");
}
