...
int main()
{
    ...
    config.layerCount = 2;
    expect(climateweather::makeState(columns, rows + 1, 2, 42).layers.empty(),
        "weather state construction must reject an extra latitude row");
    ...
    expect(std::abs(dailyStatistics.meanSpeedMps[0] - 7.225f) < 1.0e-5f &&
        std::abs(dailyStatistics.speedStandardDeviationMps[0] - std::sqrt((90.0 * 90.0 - 1.0) / (12.0 * 400.0))) < 1.0e-5,
        "cached speed statistics must preserve the analytic mean and variance of a linear series");
    ...
    const double originalMass = climategrid::areaWeightedIntegral(
        columns, rows, replayFirst.layers[0].heightAnomalyMetres);
    const double coarseMass = climategrid::areaWeightedIntegral(
        columns / 2, rows / 2, coarse.layers[0].heightAnomalyMetres);
