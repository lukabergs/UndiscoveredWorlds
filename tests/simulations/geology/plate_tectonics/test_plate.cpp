...
TEST(Plate, ClassifiesOceanicAndContinentalTypes)
{
    ...
    std::vector<uint8_t> basalt(width * height,
        platec::material::to_index(platec::material::Type::Basalt));

    for (uint32_t i = 0; i < width * height; ++i) {
        oceanicMap[i] = 0.18f;
        continentalMap[i] = oceanicMap[i];
    }

    // Material composition determines plate type independently of surface height.
    plate oceanic = plate(11, oceanicMap, width, height, 0, 0, 1,
                          WorldDimension(64, 64), basalt.data());
    plate continental =
...
TEST(Plate, OceanicBuoyancyTracksRelativeLightness)
{
    ...
    plate deepOcean =
        plate(33, deepOceanMap, width, height, 0, 0, 1, WorldDimension(64, 64), basalt.data());
    plate lightOcean =
        plate(44, lightOceanMap, width, height, 20, 20, 1, WorldDimension(64, 64), basalt.data());
...
TEST(Plate, addCrustBySubduction)
{
    ...
    const uint32_t worldPointX = 240;
    const uint32_t worldPointY = 120;
    const float* crust = nullptr;
    const uint32_t* ages = nullptr;
    p.getMap(&crust, &ages);
    const std::vector<float> before(crust, crust + 80 * 55);
    const std::vector<uint32_t> agesBefore(ages, ages + before.size());
    ...
    // Subduction now distributes crust across an arc. Check conservation and
    // rejuvenation over the footprint rather than requiring a single-cell spike.
    p.getMap(&crust, &ages);
    double deposited = 0.0;
    size_t changedCells = 0;
    size_t youngerCells = 0;
    for (size_t i = 0; i < before.size(); ++i) {
        EXPECT_GE(crust[i], before[i]);
        deposited += static_cast<double>(crust[i]) - before[i];
        changedCells += crust[i] > before[i] ? 1U : 0U;
        youngerCells += ages[i] > agesBefore[i] ? 1U : 0U;
        EXPECT_GE(ages[i], agesBefore[i]);
        EXPECT_LT(ages[i], 123U);
    }
    EXPECT_GT(changedCells, 1U);
    EXPECT_GT(youngerCells, 0U);
    EXPECT_NEAR(deposited, 0.8, 1e-5);
    EXPECT_NEAR(p.getMass(), massBefore + 0.8f, 1e-3f);
}
